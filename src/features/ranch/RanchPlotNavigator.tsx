...
const BUILDING_SHORTCUTS: BuildingShortcut[] = [
  ...
  { id: "town", plotId: "homestead", title: "Town Road", hint: "Travel to town for market, guild, and future town services.", x: 73, labelY: 81, upgradeIds: [] },
  ...
  return (
    <nav className={styles.plotNavigator} aria-label="Ranch plot navigation">
      ...
      {currentSave && ranchUpgrades ? visibleShortcuts.map((shortcut) => {
        ...
        return <button key={`${shortcut.plotId}-${shortcut.id}`} type="button" className={`${styles.buildingMarker} ${hasUpgrade ? styles.hasUpgrade : ""} ${hasAffordableUpgrade ? styles.affordableUpgrade : ""}`} style={getShortcutStyle(shortcut)} onClick={() => setSelectedShortcut(shortcut)} aria-label={`${shortcut.title} building level and upgrade shortcut`}><span>{shortcut.title}</span><em>{level === null ? "Info" : `Lv. ${level}`}</em>{hasUpgrade ? <strong aria-hidden="true">⬆</strong> : null}</button>;
      }) : null}
