...
export function RanchOfficeScreen() {
  ...
  const [message, setMessage] = useState("Welcome to the Ranch Office. Bovine, Lapine, and Equine habitats now have full upgrade tracks.");
    ...
    <main className={styles.screen}>
      <section className={styles.frame}>
        ...
        <header className={styles.header}>
          <div>
            <p className={styles.kicker}>M13.5 Ranch Office</p>
            <div className={styles.titleRow}><h1>Ranch Office</h1><button type="button" className={styles.helpButton} onClick={() => setShowHelp(true)} aria-label="Open Ranch Office help">i</button></div>
          </div>
        ...
        <section className={styles.officeOverlay} aria-label="Ranch office upgrades">
          <aside className={`${styles.panel} ${styles.panelScrollable}`}>
            ...
            <div className={styles.overviewGrid}>
              ...
              {capacityCards.map((item) => <div key={item.family} className={styles.overviewCard}><span>{HABITAT_LABELS[item.family]}</span><strong>{usedByFamily(item.family)} / {item.capacity}</strong></div>)}
            ...
            <div className={styles.detailBody}>
              ...
              <div className={styles.resourceBox}>
                ...
                <div className={styles.bonusList}>
                  ...
                  <span>Bovine capacity <strong>{effects.bovineCapacity}</strong></span>
                  <span>Lapine capacity <strong>{effects.lapineCapacity}</strong></span>
                  <span>Equine capacity <strong>{effects.equineCapacity}</strong></span>
              ...
              <div className={styles.purchaseCard}><div><span className={styles.smallLabel}>Purchase</span><strong>{nextUpgradeTier ? `Tier ${nextUpgradeTier.tier}` : "Max Tier"}</strong><p>{nextUpgradeTier ? `${formatGold(nextUpgradeTier.costGold)}${nextUpgradeTier.costGp ? ` + ${formatGuildPoints(nextUpgradeTier.costGp)}` : ""} • applies immediately` : "This upgrade is fully improved."}</p></div><button type="button" className={styles.primaryButton} disabled={!nextUpgradeTier || currentSave.currencies.gold < (nextUpgradeTier?.costGold ?? 0) || currentSave.currencies.guildPoints < (nextUpgradeTier?.costGp ?? 0)} onClick={() => setPendingUpgradeId(selectedUpgrade.upgradeId)}>{nextUpgradeTier ? "Upgrade" : "Max Tier"}</button></div>
            </div>
          </aside>
        </section>

        {showHelp ? <div className={styles.modalBackdrop} role="presentation" onClick={() => setShowHelp(false)}><section className={styles.resultModal} role="dialog" aria-modal="true" onClick={(event) => event.stopPropagation()}><img src={RANCH_UPGRADE_ASSETS.ranchLedger} alt="" /><p className={styles.kicker}>Ranch Office Help</p><h2>How Upgrades Work</h2><ul className={styles.helpList}><li>Use the left ledger tabs to switch between overview, habitats, nursery, breeding pen, and recovery upgrades.</li><li>All five habitat families now have capacity upgrades: Feline, Canine, Bovine, Lapine, and Equine.</li><li>The middle column is icon-only. Click an icon to preview its details in the right panel.</li><li>The right panel shows current tier, next tier, ranch-wide effects, and the purchase button.</li><li>Habitat and nursery capacity upgrades apply immediately.</li></ul><p className={styles.message}>{message}</p><button type="button" className={styles.primaryButton} onClick={() => setShowHelp(false)}>Got It</button></section></div> : null}

        {pendingUpgrade && pendingNextTier ? <div className={styles.modalBackdrop} role="presentation" onClick={() => setPendingUpgradeId(null)}><section className={styles.resultModal} role="dialog" aria-modal="true" onClick={(event) => event.stopPropagation()}><img src={pendingUpgrade.iconPath} alt="" /><p className={styles.kicker}>Confirm Ranch Upgrade</p><h2>{pendingUpgrade.name}</h2><p>Upgrade from Tier {pendingUpgradeTier} to Tier {pendingNextTier.tier}?</p><div className={styles.resourceBox}><span className={styles.smallLabel}>Cost</span><div className={styles.resourceGrid}><div className={styles.resource}><img src={RANCH_UPGRADE_ASSETS.gold} alt="" /><div><span>Gold</span><strong>{pendingNextTier.costGold}</strong></div></div><div className={styles.resource}><img src={RANCH_UPGRADE_ASSETS.gp} alt="" /><div><span>GP</span><strong>{pendingNextTier.costGp ?? 0}</strong></div></div></div></div><div className={styles.resourceBox}><span className={styles.smallLabel}>New Effect</span><strong>{pendingNextTier.effectLabel}</strong><p>Ranch effects apply immediately.</p></div><div className={styles.modalActions}><button type="button" className={styles.secondaryButton} onClick={() => setPendingUpgradeId(null)}>Cancel</button><button type="button" className={styles.primaryButton} onClick={confirmUpgradePurchase}>Confirm Upgrade</button></div></section></div> : null}

        {upgradeSummary ? <div className={styles.modalBackdrop} role="presentation" onClick={() => setUpgradeSummary(null)}><section className={styles.resultModal} role="dialog" aria-modal="true" onClick={(event) => event.stopPropagation()}><img src={RANCH_UPGRADE_ASSETS.ranchUpgrade} alt="" /><p className={styles.kicker}>Upgrade Complete</p><h2>{upgradeSummary.upgradeName}</h2><div className={styles.resultGrid}><span>Tier</span><strong>{upgradeSummary.oldTier} → {upgradeSummary.newTier}</strong><span>Effect</span><strong>{upgradeSummary.effectLabel}</strong><span>Spent</span><strong>{upgradeSummary.costGold} Gold • {upgradeSummary.costGp} GP</strong><span>Remaining</span><strong>{upgradeSummary.remainingGold} Gold • {upgradeSummary.remainingGp} GP</strong></div><p className={styles.message}>{upgradeSummary.immediateEffectLabel}</p><button type="button" className={styles.primaryButton} onClick={() => setUpgradeSummary(null)}>Continue</button></section></div> : null}
