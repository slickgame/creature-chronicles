...
function formatMaterials(value: number): string {
  return `${value} Materials`;
}

function formatUpgradeCost(costGold: number, costGp = 0, costMaterials = 0): string {
  return `${formatGold(costGold)}${costGp ? ` + ${formatGuildPoints(costGp)}` : ""}${costMaterials ? ` + ${formatMaterials(costMaterials)}` : ""}`;
}

function readRanchEventLog(value: boolean | number | string | undefined): string[] {
  try {
...
function formatPercentModifier(value: number): string {
  if (value > 0) return `+${value}%`;
  if (value < 0) return `${value}%`;
  return "0%";
}

function formatBreedingEnergyModifier(value: number): string {
  if (value > 0) return `-${value} energy`;
  if (value < 0) return `+${Math.abs(value)} energy cost`;
  return "0";
}
    ...
    <main className={styles.screen}>
      <section className={styles.frame}>
        ...
        <header className={styles.header}>
          <div>
            <p className={styles.kicker}>M16 Ranch Construction</p>
          ...
          <div className={styles.headerActions}>
            ...
            <div className={styles.statBox}><img src={RANCH_UPGRADE_ASSETS.ranchUpgrade} alt="" /><span>Materials</span><strong>{materialsStock}</strong></div>
        ...
        <section className={styles.officeOverlay} data-office-mode={category} aria-label="Ranch office upgrades">
          <aside className={`${styles.panel} ${styles.panelScrollable}`}>
            ...
            <div className={styles.overviewGrid}>
              ...
              <div className={styles.overviewCard}><span>Materials</span><strong>{materialsStock}</strong></div>
              <div className={styles.overviewCard}><span>Condition</span><strong>{ranchCondition}</strong></div>
              <div className={styles.overviewCard}><span>Damage</span><strong>{ranchDamage}/100</strong></div>
              <div className={styles.overviewCard}><span>Egg Timers</span><strong>{effects.nurseryPregnancyDays}d pregnancy / {effects.nurseryEggDays}d egg</strong></div>
              <div className={styles.overviewCard}><span>Breeding</span><strong>{formatPercentModifier(effects.breedingPregnancyBonus)} / {formatBreedingEnergyModifier(effects.breedingEnergyDiscount)}</strong></div>
              <div className={styles.overviewCard}><span>Chores</span><strong>{formatDiscount(effects.ranchChoreEnergyDiscount)} energy / +{effects.ranchChoreScoreBonus} score</strong></div>
            </div>
          </aside>
          ...
          {category === "overview" ? (
            <aside className={`${styles.panel} ${styles.panelScrollable}`}>
              <div className={styles.detailBody}>
                <div><span className={styles.tier}>Ranch overview</span><h2 className={styles.contractTitle}>Operations & Condition</h2><p>Use this overview for repairs, condition penalties, history, capacity, and current ranch-wide effects.</p></div>
                ...
                <div className={styles.resourceBox}><span className={styles.smallLabel}>Recent Ranch History</span>{ranchHistory.length ? <ul className={styles.historyList}>{ranchHistory.slice(0, 6).map((entry, index) => <li key={`${entry}-${index}`}>{entry}</li>)}</ul> : <p>No ranch history recorded yet. Assign chores, sleep, repair damage, or haul materials to start the log.</p>}<div className={styles.historyActions}><button type="button" className={styles.secondaryButton} onClick={() => setShowHistory(true)}>Open Full History</button></div></div>
                <div className={styles.resourceBox}><span className={styles.smallLabel}>Condition Penalties</span><div className={styles.bonusList}>{CONDITION_RULES.map((rule) => <span key={rule.label}>{rule.label} <strong>{rule.range} • {rule.penalty}</strong></span>)}</div></div>
                <div className={styles.resourceBox}><span className={styles.smallLabel}>Capacity & Timers</span><div className={styles.bonusList}>{capacityCards.map((item) => <span key={item.family}>{HABITAT_LABELS[item.family]} <strong>{usedByFamily(item.family)} / {item.capacity}</strong></span>)}<span>Egg capacity <strong>{activeEggs} / {effects.nurseryEggCapacity}</strong></span><span>Pregnancy timer <strong>{effects.nurseryPregnancyDays} day{effects.nurseryPregnancyDays === 1 ? "" : "s"}</strong></span><span>Egg incubation <strong>{effects.nurseryEggDays} day{effects.nurseryEggDays === 1 ? "" : "s"}</strong></span></div></div>
            ...
            <>
              <section className={`${styles.panel} ${styles.upgradePanel}`}>
                <h2>{getRanchUpgradeCategoryLabel(category)}</h2>
                <div className={styles.upgradeList}>{categoryUpgrades.map((definition) => <button key={definition.upgradeId} type="button" aria-label={`${definition.name}. ${definition.description}`} title={definition.name} className={`${styles.upgradeButton} ${selectedUpgrade.upgradeId === definition.upgradeId ? styles.active : ""}`} onClick={() => setSelectedUpgradeId(definition.upgradeId)}><img src={definition.iconPath} alt="" /></button>)}</div>
              </section>
        ...
        {showHistory ? <div className={styles.modalBackdrop} role="presentation" onClick={() => setShowHistory(false)}><section className={styles.resultModal} role="dialog" aria-modal="true" onClick={(event) => event.stopPropagation()}><img src={RANCH_UPGRADE_ASSETS.ranchLedger} alt="" /><p className={styles.kicker}>Ranch History</p><h2>Operations Log</h2><p>Recent chores, feeding results, security events, material hauling, upkeep, and manual repairs.</p>{ranchHistory.length ? <ul className={styles.fullHistoryList}>{ranchHistory.map((entry, index) => <li key={`${entry}-${index}`}>{entry}</li>)}</ul> : <p>No ranch history recorded yet. Sleep after assigning chores to generate the first entries.</p>}<button type="button" className={styles.primaryButton} onClick={() => setShowHistory(false)}>Close History</button></section></div> : null}
        {showHelp ? <div className={styles.modalBackdrop} role="presentation" onClick={() => setShowHelp(false)}><section className={styles.resultModal} role="dialog" aria-modal="true" onClick={(event) => event.stopPropagation()}><img src={RANCH_UPGRADE_ASSETS.ranchLedger} alt="" /><p className={styles.kicker}>Ranch Office Help</p><h2>How Upgrades Work</h2><ul className={styles.helpList}><li>Use Ranch Overview for repairs, condition penalties, history, and ranch-wide effects.</li><li>Use the other ledger tabs to upgrade one system without the repair/history clutter.</li><li>Tier 1 upgrades cost Gold only. Tier 2+ upgrades may cost Gold, GP, and Ranch Materials.</li><li>Assign Field Hauling before sleep to produce Materials and automatically repair some damage with Upkeep score.</li><li>Use Manual Repair from Ranch Overview to spend {RANCH_REPAIR_MATERIAL_COST} Materials and repair up to {RANCH_REPAIR_DAMAGE_AMOUNT} damage immediately.</li></ul><p className={styles.message}>{message}</p><button type="button" className={styles.primaryButton} onClick={() => setShowHelp(false)}>Got It</button></section></div> : null}
