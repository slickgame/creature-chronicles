...
export function RanchOfficeScreen() {
  ...
  return <main className={styles.screen}><section className={styles.frame}><div className={styles.backgroundArt} aria-hidden="true" /><div className={styles.shade} aria-hidden="true" /><header className={styles.header}><div><p className={styles.kicker}>M14 Ranch Office</p><div className={styles.titleRow}><h1>Ranch Office</h1><button type="button" className={styles.helpButton} onClick={() => setShowHelp(true)} aria-label="Open Ranch Office help">i</button></div></div><div className={styles.headerActions}><div className={styles.statBox}><img src={RANCH_UPGRADE_ASSETS.gold} alt="" /><span>Gold</span><strong>{formatGold(currentSave.currencies.gold)}</strong></div><div className={styles.statBox}><img src={RANCH_UPGRADE_ASSETS.gp} alt="" /><span>Guild Points</span><strong>{formatGuildPoints(currentSave.currencies.guildPoints)}</strong></div><div className={styles.statBox}><img src={RANCH_UPGRADE_ASSETS.ranchUpgrade} alt="" /><span>Materials</span><strong>{materialsStock}</strong></div><button type="button" onClick={goToRanch}>Back to Ranch</button><button type="button" onClick={goToMainMenu}>Main Menu</button></div></header>
    <section className={styles.officeOverlay} aria-label="Ranch office upgrades"><aside className={`${styles.panel} ${styles.panelScrollable}`}><h2>Ledger Tabs</h2><div className={styles.filters}>{CATEGORIES.map((item) => <button key={item.id} type="button" className={`${styles.filterButton} ${category === item.id ? styles.active : ""}`} onClick={() => handleCategoryClick(item.id)}><img src={item.icon} alt="" />{item.label}</button>)}</div><div className={styles.overviewGrid}><div className={styles.overviewCard}><span>Total Upgrades</span><strong>{totalUpgradeTiers} tiers</strong></div><div className={styles.overviewCard}><span>Materials</span><strong>{materialsStock}</strong></div><div className={styles.overviewCard}><span>Condition</span><strong>{ranchCondition}</strong></div><div className={styles.overviewCard}><span>Penalty</span><strong>{currentConditionRule.penalty}</strong></div><div className={styles.overviewCard}><span>Damage</span><strong>{ranchDamage}/100</strong></div>{capacityCards.map((item) => <div key={item.family} className={styles.overviewCard}><span>{HABITAT_LABELS[item.family]}</span><strong>{usedByFamily(item.family)} / {item.capacity}</strong></div>)}<div className={styles.overviewCard}><span>Egg Slots</span><strong>{activeEggs} / {effects.nurseryEggCapacity}</strong></div></div></aside>
    ...
    <footer className={styles.versionFooter}>{version}</footer></section></main>;
}
