...
const panelStyle: CSSProperties = { width: "min(1120px, 100%)", maxHeight: "90vh", overflow: "auto", display: "grid", gridTemplateColumns: "minmax(250px,.74fr) minmax(360px,1.26fr)", gap: 14, padding: 18, border: "3px solid rgba(245,201,128,.88)", borderRadius: 22, background: "linear-gradient(rgba(44,29,22,.98),rgba(12,9,8,.98))", color: "#fff7dd", boxShadow: "0 28px 70px rgba(0,0,0,.7)" };
...
export function StoryLogOverlay() {
  ...
  return <div style={backdropStyle} role="presentation" onClick={() => setOpen(false)}><section style={panelStyle} role="dialog" aria-modal="true" aria-labelledby="story-log-title" onClick={(event) => event.stopPropagation()}><aside style={listStyle}><div><p style={kickerStyle}>Ranch Office Codex</p><h2 id="story-log-title" style={{ margin: "4px 0", color: "#fff", fontSize: "2rem", lineHeight: 1 }}>Story Log</h2><p style={bodyStyle}>Reread Chapter 1 scenes you have unlocked. Locked entries show what to do next.</p></div><div style={{ display: "grid", gap: 7 }}>{entries.map((entry) => <button key={entry.id} type="button" onClick={() => setSelectedId(entry.id)} style={{ textAlign: "left", padding: 10, border: `1px solid ${selected?.id === entry.id ? "rgba(127,219,255,.72)" : entry.seen ? "rgba(245,201,128,.34)" : "rgba(120,120,120,.26)"}`, borderRadius: 12, background: selected?.id === entry.id ? "rgba(127,219,255,.12)" : "rgba(0,0,0,.24)", color: entry.seen ? "#fff7dd" : "#a99d8b", fontWeight: 900 }}><span style={{ display: "block", color: entry.seen ? "#7fdbff" : "#8d8376", fontSize: ".68rem", textTransform: "uppercase", letterSpacing: ".1em" }}>{entry.seen ? "Unlocked" : "Locked"} • {entry.kind}</span>{entry.title}</button>)}</div><button type="button" style={buttonStyle} onClick={() => setOpen(false)}>Close Log</button></aside>{selected ? <StoryLogDetail entry={selected} /> : null}</section></div>;
}

function StoryLogDetail({ entry }: { entry: StoryLogEntry }) {
  return <article style={{ display: "grid", gap: 12, alignContent: "start", padding: 14, border: "1px solid rgba(245,201,128,.3)", borderRadius: 18, background: "rgba(0,0,0,.22)" }}><header style={{ display: "grid", gridTemplateColumns: "82px 1fr", gap: 12, alignItems: "center" }}><img src={entry.portraitPath} alt="" style={{ width: 74, height: 74, objectFit: "contain", border: "2px solid rgba(127,219,255,.58)", borderRadius: 16, padding: 6, background: "rgba(255,247,221,.08)", opacity: entry.seen ? 1 : .45 }} /><div><p style={kickerStyle}>{entry.seen ? "Recovered Record" : "Locked Record"}</p><h2 style={{ margin: "4px 0", color: "#fff", fontSize: "clamp(1.6rem,3vw,2.6rem)", lineHeight: 1 }}>{entry.title}</h2><span style={{ color: "#7fdbff", fontWeight: 950 }}>{entry.speaker}</span></div></header>{entry.seen ? <div style={{ display: "grid", gap: 10 }}>{entry.lines.map((line, index) => <p key={`${entry.id}-${index}`} style={bodyStyle}>{line}</p>)}</div> : <div style={{ padding: 12, border: "1px solid rgba(127,219,255,.28)", borderRadius: 14, background: "rgba(127,219,255,.08)" }}><p style={bodyStyle}>This record has not been unlocked yet.</p><p style={{ ...bodyStyle, color: "#7fdbff", marginTop: 8 }}>{entry.lockedReason ?? "Continue Chapter 1 to unlock this scene."}</p></div>}</article>;
}
