...
const ICONS = { shop: "/images/ui/icons/icon_shop_bag.png", reroll: "/images/ui/icons/icon_reroll.png", price: "/images/ui/icons/icon_price_tag.png", sold: "/images/ui/icons/icon_sold.png", gold: "/images/ui/currency/icon_currency_gold.png", tamsin: "/images/npcs/town/tamsin_vale_portrait.png" } as const;
...
export function MarketScreen() {
  ...
  const [message, setMessage] = useState("Tamsin has this week's adoption listings ready.");
  ...
  return <main className={styles.screen}><section className={styles.frame}><div className={styles.backgroundArt} aria-hidden="true" /><div className={styles.shade} aria-hidden="true" /><header className={styles.header}><div><p className={styles.kicker}>M37 Trust Bonuses</p><h1>Vale's Adoption Hearth Lv. {adoptionLevel}</h1><p>Adopt weekly creature placements or pay Gold to welcome new arrivals.</p><p className={styles.message}>{message}</p></div><div className={styles.headerActions}><div className={styles.statBox}><span>Gold</span><strong>{formatGold(currentSave.currencies.gold)}</strong></div><div className={styles.statBox}><span>GP</span><strong>{formatGuildPoints(currentSave.currencies.guildPoints)}</strong></div><button type="button" className={styles.backButton} onClick={goToTown}>Back to Town</button><button type="button" className={styles.backButton} onClick={goToMainMenu}>Main Menu</button></div></header><section className={styles.grid}><aside className={styles.panel}><h2>Tamsin Vale</h2><div className={styles.sideList}><div className={styles.infoCard}><img src={ICONS.tamsin} alt="" onError={(event) => { event.currentTarget.src = ICONS.shop; }} /><span>Adoption Steward</span><strong>Tamsin Vale</strong></div><div className={styles.infoCard}><span>Trust</span><strong>{getNpcTrustSummary(currentSave, "tamsin_vale")}</strong></div><div className={styles.infoCard}><span>Next Unlock</span><strong>{getNpcNextUnlock(currentSave, "tamsin_vale")}</strong></div><div className={styles.infoCard}><span>Restock Week</span><strong>Week {market.weekNumber}</strong></div><div className={styles.infoCard}><img src={ICONS.shop} alt="" /><span>Adoption Listings</span><strong>{market.listings.length} / {marketEffects.marketListingCount}</strong></div><div className={styles.infoCard}><img src={ICONS.reroll} alt="" /><span>New Arrivals Cost</span><strong>{formatGold(rerollCost)}</strong></div><div className={styles.infoCard}><span>Adoption Network</span><strong>{(marketEffects.marketVariantChance * 100).toFixed(2)}% base special chance • {Math.round(marketEffects.marketRerollDiscount * 100)}% arrival discount</strong></div><div className={styles.infoCard}><span>Tamsin's Note</span><strong>{TAMSIN_INTRO}</strong></div><div className={styles.infoCard}><span>Rules</span><strong>Habitat capacity is checked before adoption.</strong></div><button type="button" className={styles.rerollButton} onClick={handleReroll} disabled={currentSave.currencies.gold < rerollCost}>Refresh Arrivals</button></div></aside><section className={styles.panel} aria-label="Adoption listings"><h2>Adoption Listings</h2><div className={styles.listings}>{market.listings.map((listing) => { const variant = getVariantDefinition(listing.variantId); const isSold = listing.status === "sold"; const listingPrice = getMarketListingPrice(syncedSave, listing); const canAfford = currentSave.currencies.gold >= listingPrice; const preview = getMarketListingPreview(syncedSave, listing); const bestGrades = Object.values(preview.statGrades).filter((grade) => grade === "A" || grade === "S").length; return <article key={listing.listingId} className={`${styles.listing} ${isSold ? styles.sold : ""}`}><div className={styles.listingArt}><img src={getMarketListingImage(listing)} alt="" /></div><div className={styles.listingBody}><div className={styles.listingHeaderRow}><div><span className={styles.listingMeta}>{variant.rarity} • {variant.family}</span><h3 className={styles.listingName}>{variant.name}</h3></div><button type="button" className={styles.infoButton} onClick={() => setActiveListing(listing)} aria-label={`View ${variant.name} details`}>i</button></div><p className={styles.listingDesc}>{getMarketListingDescription(listing)}</p><p className={styles.gradePreview}>{bestGrades > 0 ? `${bestGrades} premium stat grade${bestGrades === 1 ? "" : "s"}` : "Standard stat grade spread"}</p><div className={styles.price}><img src={ICONS.price} alt="" /><div><span>Adoption Fee</span><strong>{formatGold(listingPrice)}</strong>{listingPrice < listing.price ? <em>Trust discount from {formatGold(listing.price)}</em> : null}</div></div><div className={styles.actions}>{isSold ? <span className={styles.soldBadge}><img src={ICONS.sold} alt="" /> Adopted</span> : <button type="button" className={styles.buyButton} onClick={() => handleBuy(listing)} disabled={!canAfford}>Adopt</button>}</div></div></article>; })}</div></section></section></section>{activeListing ? <MarketListingModal save={syncedSave} listing={activeListing} canAfford={currentSave.currencies.gold >= getMarketListingPrice(syncedSave, activeListing)} onBuy={handleBuy} onClose={() => setActiveListing(null)} /> : null}</main>;
}
...
function MarketListingModal({ save, listing, canAfford, onBuy, onClose }: { save: GameSave; listing: MarketListing; canAfford: boolean; onBuy: (listing: MarketListing) => void; onClose: () => void }) {
  ...
  const listingPrice = getMarketListingPrice(save, listing);
