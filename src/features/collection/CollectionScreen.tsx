...
export function CollectionScreen() {
  ...
  function handleSelect(creature: CreatureRecord) { setSelectedCreatureId(creature.creatureId); setRenameValue(creature.nickname); setConfirmMode(null); }
  ...
  return (
    <main className={styles.screen}>
      <section className={styles.frame}>
        <header className={styles.header}><div><p className={styles.kicker}>M13 Creature Collection</p><h1>Collection Tracker</h1><p>{message}</p></div><div className={styles.headerActions}><button type="button" onClick={goToRanch}>Back to Ranch</button></div></header>
        ...
        <section className={styles.contentGrid}>
          <aside className={styles.listPanel}>
            ...
            <div className={styles.creatureList}>{visibleCreatures.map((creature) => { const variant = getVariantDefinition(creature.variantId); const species = getSpeciesDefinition(creature.speciesId); return <button key={creature.creatureId} type="button" className={`${styles.creatureCard} ${selectedCreature?.creatureId === creature.creatureId ? styles.selectedCard : ""}`} onClick={() => handleSelect(creature)}><img src={variant.portraitPath || CREATURE_PLACEHOLDER_IMAGE} alt="" onError={(event) => { event.currentTarget.src = CREATURE_PLACEHOLDER_IMAGE; }} /><div><strong>{creature.nickname}{creature.isLocked ? " 🔒" : ""}</strong><span>{variant.name} {species.name} • Lv {creature.level}</span><em>{variant.rarity} • Best Grade {getHighestStatGrade(creature)}</em></div><img className={styles.originMiniIcon} src={getOriginIcon(creature.origin)} alt="" /></button>; })}</div>
          </aside>
          <section className={styles.detailPanel}>{selectedCreature ? <CreatureDetail creature={selectedCreature} renameValue={renameValue || selectedCreature.nickname} setRenameValue={setRenameValue} onRename={handleRename} onToggleLock={() => toggleCreatureLock(selectedCreature.creatureId)} onConfirm={setConfirmMode} /> : <div className={styles.noSelection}>No creature selected.</div>}</section>
        </section>
      </section>
