...
export function HabitatScreen() {
  ...
  const creatures = useMemo(() => {
    ...
    return currentSave.creatures.filter(
      (creature) =>
        getVariantDefinition(creature.variantId).family === activeHabitatFamily,
    );
  ...
  function handleToggleLock() {
    ...
    setMessage(
      selectedCreature.isLocked
        ? `${selectedCreature.nickname} was unlocked.`
        : `${selectedCreature.nickname} is now locked and protected.`,
    );
  }
...
function CreatureProfile({
  ...
  dayState: Parameters<typeof getEstimatedDeliveryDateLabel>[0];
  dayNumber: number;
  onDonate: () => void;
  onFeed: () => void;
  onRelease: () => void;
