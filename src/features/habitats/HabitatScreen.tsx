...
export function HabitatScreen() {
  ...
  function handleSelectCreature(creature: CreatureRecord) { setSelectedCreatureId(creature.creatureId); setRenameValue(creature.nickname); setMessage(`${creature.nickname} selected.`); }
  ...
  return <main className={styles.screen}><section className={styles.frame}><header className={styles.header}><div><p className={styles.kicker}>M13 Habitat Management</p><h1>{habitatTitle}</h1><p>{habitatDescription}</p></div><div className={styles.headerActions}><div className={styles.capacityCard}><span>Capacity</span><strong>{creatures.length} / {habitat.capacity}</strong></div><button type="button" onClick={goToCollection}>Collection Tracker</button><button type="button" onClick={goToRanch}>Back to Ranch</button></div></header><section className={styles.contentGrid}><aside className={styles.creatureList} aria-label="Creature list"><h2>Creatures</h2><p className={styles.helpText}>{message}</p><div className={styles.cards}>{creatures.map((creature) => { const variant = getVariantDefinition(creature.variantId); const species = getSpeciesDefinition(creature.speciesId); const isSelected = selectedCreature?.creatureId === creature.creatureId; return <button key={creature.creatureId} type="button" className={`${styles.creatureCard} ${isSelected ? styles.selectedCard : ""}`} onClick={() => handleSelectCreature(creature)}><img src={getImagePath(variant.portraitPath)} alt="" onError={(event) => { event.currentTarget.src = CREATURE_PLACEHOLDER_IMAGE; }} /><div><strong>{creature.nickname}{creature.isLocked ? " 🔒" : ""}</strong><span>{variant.name} {species.name} • Lv {creature.level}</span><em>{variant.rarity} • {creature.originLabel}</em></div></button>; })}</div></aside><section className={styles.profilePanel} aria-label="Creature profile">{selectedCreature ? <CreatureProfile creature={selectedCreature} onDonate={handleDonate} onFeed={handleFeed} onRelease={handleRelease} onRename={handleRename} onToggleLock={handleToggleLock} renameValue={renameValue || selectedCreature.nickname} setRenameValue={setRenameValue} /> : <div className={styles.noSelection}>No creature selected.</div>}</section></section></section></main>;
}

function CreatureProfile({ creature, onDonate, onFeed, onRelease, onRename, onToggleLock, renameValue, setRenameValue }: { creature: CreatureRecord; onDonate: () => void; onFeed: () => void; onRelease: () => void; onRename: () => void; onToggleLock: () => void; renameValue: string; setRenameValue: (value: string) => void }) {
