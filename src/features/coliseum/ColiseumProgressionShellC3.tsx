...
export function ColiseumProgressionScreen() {
  ...
  if (mode === "arena") {
    return <div className={styles.shell}>{toolbar}<ColiseumProgressionScreenC2 /></div>;
  }
...
function CreatureSelector({ creatures, selectedCreatureId, onSelect }: { creatures: NonNullable<ReturnType<typeof useGameContext>["currentSave"]>["creatures"]; selectedCreatureId: CreatureId | null; onSelect: (id: CreatureId) => void }) {
...
function TechniqueDesk({ save, selectedCreature, selectedCreatureId, setSelectedCreatureId, replacementByMove, setReplacementByMove, onResult }: { save: NonNullable<ReturnType<typeof useGameContext>["currentSave"]>; selectedCreature: NonNullable<ReturnType<typeof useGameContext>["currentSave"]>["creatures"] extends Array<infer T> ? T | null : never; selectedCreatureId: CreatureId | null; setSelectedCreatureId: (id: CreatureId) => void; replacementByMove: Record<string, string>; setReplacementByMove: (value: Record<string, string>) => void; onResult: (result: ColiseumC3Result) => void }) {
...
function ContractHold({ save, onResult, onExchange }: { save: NonNullable<ReturnType<typeof useGameContext>["currentSave"]>; onResult: (result: ColiseumC3Result) => void; onExchange: () => void }) {
  const state = getColiseumC3State(save);
...
function RewardHistory({ save }: { save: NonNullable<ReturnType<typeof useGameContext>["currentSave"]> }) {
  const state = getColiseumC3State(save);
