...
export type BattleUiPhase = "team-selection" | "battle" | "result";

type UsedCombatItems = {
  tacticsKit: boolean;
  fieldTonic: boolean;
  revivalSalve: boolean;
};

const EMPTY_USED_ITEMS: UsedCombatItems = {
  tacticsKit: false,
  fieldTonic: false,
  revivalSalve: false,
};
...
const supportPanelStyle = {
  marginTop: 12,
  padding: 12,
  border: "1px solid rgba(245, 201, 128, 0.45)",
  borderRadius: 10,
  background: "rgba(10, 18, 25, 0.78)",
  color: "#fff7dd",
} as const;

export function BattleArenaScreenOutfitter() {
  const { currentSave, goToBattleOutfitter, goToTown, saveCurrentGame } = useGameContext();
  ...
  const availableRoster = useMemo(
    () => currentSave ? roster.filter((creature) => !getUnavailableReason(currentSave, creature)) : [],
    [currentSave, roster],
  );
  ...
  function startBattle() {
    ...
    if (armTacticsKit && tacticsStock <= 0) {
      setMessage("No Team Tactics Kit is available. Visit the Battle Outfitter or enter without it.");
      return;
    }
    ...
    setPhase("battle");
    setMessage(`Equipment and Focus training are active. Select a target, then choose one compatible move. The ${getBattleAiDifficultyLabel(aiDifficulty)} AI plans after your queue is confirmed.`);
  }
  ...
  function chooseMove(moveId: string) {
    ...
    setSelectedTarget(null);
    setMessage(nextActor ? `Action queued. Select a target for ${battleState.combatants[nextActor].name}.` : "All player actions are queued. Review them, use support items if needed, then confirm the round.");
  }
  ...
  function useSupportItem(item: "tonic" | "revival") {
    if (!battleState || selectedTarget?.kind !== "combatant") {
      setMessage("Select a ranch-team creature before using a support item.");
      return;
    }
    const targetId = selectedTarget.combatantId;
    const result = item === "tonic"
      ? useFieldTonic(currentSave, battleState, targetId)
      : useRevivalSalve(currentSave, battleState, targetId);
    if (!result.ok) {
      setMessage(result.message);
      return;
    }
    ...
    setUsedItems((current) => ({
      ...current,
      fieldTonic: current.fieldTonic || item === "tonic",
      revivalSalve: current.revivalSalve || item === "revival",
    }));
    setMessage(result.message);
        ...
        <section className={styles.frame}>
          ...
          <section className={styles.selectionSummary} data-ui-text-box="auto">
            ...
            <div>
              ...
              <button
                type="button"
                className={armTacticsKit ? styles.confirmButton : styles.secondaryButton}
                onClick={() => setArmTacticsKit((value) => !value)}
                disabled={tacticsStock <= 0}
              >
                {armTacticsKit ? "Tactics Kit Armed" : `Use Tactics Kit (${tacticsStock})`}
              </button>
              <small>Start Inspired and gain 10 Battle Energy.</small>
            </div>
          </section>
          <section className={styles.rosterGrid}>{roster.map((creature) => <TeamSelectionCard key={creature.creatureId} creature={creature} selected={effectiveSelection.includes(creature.creatureId)} unavailableReason={getUnavailableReason(currentSave, creature)} readinessLabel={getBattleReadinessLabel(currentSave, creature.creatureId)} onToggle={() => toggleCreature(creature)} />)}</section>
          <footer className={styles.selectionFooter}><p>Assigned equipment, Focus training, and an optional Team Tactics Kit now affect the exhibition. Rewards and long-term battle consequences remain deferred to Coliseum progression.</p><button type="button" onClick={startBattle} disabled={effectiveSelection.length !== 3}>Enter Exhibition</button></footer>
        </section>
      </main>
    );
  }
      ...
      <section className={styles.frame}>
        ...
        <section className={styles.commandDeck}>
          <div className={styles.actionPanel}>
            ...
            {phase === "result" ? <div className={styles.resultPanel}><h2>{battleState.outcome === "player_won" ? "Victory" : battleState.outcome === "enemy_won" ? "Defeat" : "Draw"}</h2><p>Equipment and consumed support items affected this preview. Coliseum rewards and persistent combat records remain a later patch.</p>{battleState.outcome === "enemy_won" && !usedItems.revivalSalve && revivalStock > 0 ? <p>Select a fainted ranch creature and use a Revival Salve to resume.</p> : null}<button type="button" onClick={resetToSelection}>Start Another Exhibition</button></div> : selectedTarget && activeActor ? <div className={styles.moveGrid}>{compatibleMoves.length ? compatibleMoves.map((option) => <button key={option.move.id} type="button" className={`${styles.moveButton} ${styles[`category_${option.move.category}`]}`} onClick={() => chooseMove(option.move.id)} disabled={!option.usable}><span className={styles.moveTitle}><strong>{option.move.name}</strong><em>{option.move.category}</em></span><span className={styles.moveNumbers}>PWR {option.move.power} · ACC {option.move.accuracy}% · BE {option.move.battleEnergyCost} · CD {activeActor.cooldowns[option.move.id] ?? 0}/{option.move.cooldown}</span><span>{getBattleTargetTypeLabel(option.move.targetType)} · {moveEffectLabel(option.move)}</span>{option.reason ? <small>{option.reason}</small> : <small>Ready</small>}</button>) : <div className={styles.emptyMoveState}><strong>No compatible equipped moves</strong><p>Select a different target type for {activeActor.name}, or use a support item on a ranch creature.</p></div>}</div> : <div className={styles.emptyMoveState}><strong>Target first</strong><p>Select a living enemy, ally, the active creature itself, a fainted ranch creature for Revival, or the battlefield.</p></div>}

            <section style={supportPanelStyle} data-ui-text-box="auto">
              <div className={styles.panelHeading}><div><span>Battle Outfitter</span><strong>Support Items</strong></div></div>
              <p className={styles.statusLine}>Select a ranch creature before using an item. Each item type may be used once in this exhibition.</p>
              <div className={styles.cardActions}>
                <button type="button" onClick={() => useSupportItem("tonic")} disabled={usedItems.fieldTonic || tonicStock <= 0 || phase === "result"}>Field Tonic ({tonicStock})</button>
                <button type="button" onClick={() => useSupportItem("revival")} disabled={usedItems.revivalSalve || revivalStock <= 0}>Revival Salve ({revivalStock})</button>
              </div>
              <small>Field Tonic restores 30% HP and 20% Battle Energy. Revival Salve restores a fainted ally at 35% HP and clears statuses.</small>
            </section>
          </div>
