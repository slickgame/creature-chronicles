...
function FocusedPairCard({ role, participant, onChoose, onInfo }: { role: string; participant: BreedingParticipant | null; onChoose: () => void; onInfo: (participant: BreedingParticipant) => void }) {
  return <article style={pairCardStyle}>{participant ? <button type="button" style={infoButtonStyle} onClick={() => onInfo(participant)} aria-label={`Inspect ${participant.displayName}`}>i</button> : null}<button type="button" style={chooseCardButtonStyle} onClick={onChoose} aria-label={`Choose ${role}`} /><div><p style={smallLabelStyle}>{role}</p><strong style={{ display: "block", color: "#fff7dd", fontSize: "1.25rem" }}>{participant?.displayName ?? `Choose ${role}`}</strong></div>{participant ? <div style={artWrapStyle}><img src={getParticipantImage(participant)} alt="" style={artStyle} onError={(event) => { event.currentTarget.src = CREATURE_PLACEHOLDER_IMAGE; }} /></div> : <div style={{ display: "grid", placeItems: "center", minHeight: 280, color: "#7fdbff", fontWeight: 950 }}>Click to select</div>}<div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 6 }}><MiniReadout label="Energy" value={participant ? formatEnergy(participant.energy, participant.maxEnergy) : "—"} /><MiniReadout label="Hearts" value={participant ? `${participant.hearts}/${participant.maxHearts}` : "—"} /><MiniReadout label="Status" value={participantStatus(participant)} /></div></article>;
}
...
function ParticipantInfoModal({ participant, creature, dayNumber, onClose }: { participant: BreedingParticipant; creature: CreatureRecord | null; dayNumber: number; onClose: () => void }) {
  if (creature) return <div style={modalBackdropStyle} role="presentation" onClick={onClose}><section style={{ ...modalStyle, width: "min(100%, 1040px)" }} role="dialog" aria-modal="true" onClick={(event) => event.stopPropagation()}><header style={{ display: "flex", justifyContent: "space-between", gap: 10, alignItems: "start" }}><div><p style={smallLabelStyle}>Breeding Inspect</p><h2 style={{ margin: 0, color: "#fff7dd", fontSize: "2.4rem" }}>{creature.nickname}</h2><p style={{ ...bodyTextStyle, color: "#7fdbff" }}>{participantStatus(participant)}</p></div><button type="button" style={buttonStyle} onClick={onClose}>Close</button></header><SharedCreatureDetail creature={creature} dayNumber={dayNumber} showActions={false} /></section></div>;
  const stats = participant.stats ? Object.entries(participant.stats) : [];
  return <div style={modalBackdropStyle} role="presentation" onClick={onClose}><section style={{ ...modalStyle, width: "min(100%, 760px)" }} role="dialog" aria-modal="true" onClick={(event) => event.stopPropagation()}><header style={{ display: "grid", gridTemplateColumns: "160px 1fr auto", gap: 14, alignItems: "center" }}><img src={getParticipantImage(participant)} alt="" style={{ width: 150, height: 150, objectFit: "contain", border: "2px solid rgba(127,219,255,.34)", borderRadius: 16, background: "rgba(0,0,0,.24)", padding: 8 }} onError={(event) => { event.currentTarget.src = CREATURE_PLACEHOLDER_IMAGE; }} /><div><p style={smallLabelStyle}>{participant.kind}</p><h2 style={{ margin: "2px 0", color: "#fff7dd", fontSize: "2.2rem" }}>{participant.displayName}</h2><p style={{ ...bodyTextStyle, color: "#7fdbff" }}>{participant.familyLabel}</p><p style={bodyTextStyle}>{participant.description ?? "No profile description available yet."}</p></div><button type="button" style={buttonStyle} onClick={onClose}>Close</button></header><div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(150px,1fr))", gap: 8 }}><StatBox label="Energy" value={formatEnergy(participant.energy, participant.maxEnergy)} /><StatBox label="Hearts" value={`${participant.hearts}/${participant.maxHearts}`} /><StatBox label="Status" value={participantStatus(participant)} /><StatBox label="Level" value={String(participant.level ?? "—")} /><StatBox label="Affection" value={String(participant.affection)} /><StatBox label="XP" value={typeof participant.xp === "number" ? `${participant.xp}/${participant.xpToNext ?? "—"}` : "—"} /></div>{stats.length ? <section style={{ display: "grid", gap: 8 }}><p style={smallLabelStyle}>Stats</p><div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(120px,1fr))", gap: 8 }}>{stats.map(([key, value]) => <StatBox key={key} label={key} value={String(value)} />)}</div></section> : null}</section></div>;
}

function BreedingResultModal({ result, onClose }: { result: BreedingAttemptRecord; onClose: () => void }) {
  const [page, setPage] = useState<ResultPage>("process");
  ...
  return <div style={modalBackdropStyle} role="presentation" onClick={onClose}><section style={modalStyle} role="dialog" aria-modal="true" onClick={(event) => event.stopPropagation()}><header style={{ display: "flex", justifyContent: "space-between", gap: 10, alignItems: "center" }}><div><p style={smallLabelStyle}>{page === "process" ? "Pairing Scene" : "Outcome Scene"}</p><h2 style={{ margin: 0, color: "#fff7dd", fontSize: "2rem" }}>{page === "process" ? `${result.giverName} × ${result.receiverName}` : success ? "Pregnancy Signs" : blocked ? "Pregnancy Blocked" : "No Pregnancy"}</h2></div><button type="button" style={buttonStyle} onClick={onClose}>Close</button></header><img src={page === "process" ? result.pairingImagePath : result.outcomeImagePath} alt="" style={{ width: "100%", maxHeight: 330, objectFit: "contain", padding: 14, border: "2px solid rgba(127,219,255,.34)", borderRadius: 18, background: "rgba(0,0,0,.28)" }} onError={(event) => { event.currentTarget.src = fallback; }} />{page === "process" ? <><p style={bodyTextStyle}>{result.processText}</p><div style={{ display: "grid", gridTemplateColumns: "repeat(6,minmax(0,1fr))", gap: 8 }}><StatBox label="Chance" value={`${result.pregnancyChance}%`} /><StatBox label="Energy Cost" value={String(result.energyCost)} /><StatBox label="Giver Energy" value={`${result.giverEnergyBefore}→${result.giverEnergyAfter}`} /><StatBox label="Receiver Energy" value={`${result.receiverEnergyBefore}→${result.receiverEnergyAfter}`} /><StatBox label="Creature XP" value={`+${result.xpGain}`} /><StatBox label="Breeder XP" value={`+${result.breederXpGain}`} /></div><button type="button" style={primaryButtonStyle} onClick={() => setPage("outcome")}>Next: Outcome</button></> : <><p style={bodyTextStyle}>{result.resultText}</p><p style={bodyTextStyle}>{result.outcomeFlavorText}</p><div style={{ display: "grid", gridTemplateColumns: "repeat(6,minmax(0,1fr))", gap: 8 }}><StatBox label="Receiver" value={success ? "Pregnant" : "Not pregnant"} /><StatBox label="Chance Used" value={`${result.pregnancyChance}%`} /><StatBox label="Pair Streak" value={String(result.streakAfter)} /><StatBox label="Progression" value={`${result.progressionEvents.length} record${result.progressionEvents.length === 1 ? "" : "s"}`} /><StatBox label="Heart Cost" value={String(result.heartCost)} /><StatBox label="Outcome" value={blocked ? "Blocked" : result.outcome} /></div><div style={{ display: "flex", gap: 8, justifyContent: "end" }}><button type="button" style={ghostButtonStyle} onClick={() => setPage("process")}>Back</button><button type="button" style={primaryButtonStyle} onClick={onClose}>Confirm</button></div></>}</section></div>;
}
