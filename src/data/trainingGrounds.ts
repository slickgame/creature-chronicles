...
export type TrainingUpgrade = { upgradeId: TrainingUpgradeId; name: string; description: string; costGold: number; materialCost: number; iconPath: string; effectLabel: string; requiredUpgradeId?: TrainingUpgradeId };
...
export const TRAINING_UPGRADES: TrainingUpgrade[] = [
  ...
  { upgradeId: "coaching_bench_upgrade", name: "Upgrade Coaching Bench", description: "Adds better measurement tools and repeatable coaching routines.", costGold: 500, materialCost: 12, iconPath: "/images/ui/icons/icon_stat_growth.png", effectLabel: "Stat Coaching chance increases from 18% to 25%." },
  { upgradeId: "assistant_trainer", name: "Hire Assistant Trainer", description: "An assistant coach lets Rhea supervise more than one creature at a time.", costGold: 650, materialCost: 16, iconPath: "/images/ui/icons/icon_service_permit.png", effectLabel: "Training capacity increases from 1 creature to 2 creatures.", requiredUpgradeId: "training_yard_upgrade" },
  { upgradeId: "advanced_training_license", name: "Advanced Training License", description: "Unlocks targeted stat coaching for advanced improvement plans.", costGold: 850, materialCost: 20, iconPath: "/images/ui/icons/icon_stat_grade.png", effectLabel: "Unlocks Focused Stat Coaching: choose STR / DEX / STA / CHA / WIL / FER.", requiredUpgradeId: "coaching_bench_upgrade" },
];

export const TRAINING_FOCI: TrainingFocus[] = [
  { focusId: "level_drill", name: "Level Drill", category: "XP", description: "Leave a creature with Rhea for a short fundamentals program focused on reliable XP gain.", costGold: 80, durationDays: 1, iconPath: "/images/ui/icons/icon_breeder_level.png", effectLabel: "Takes 1 day. On return: +35 XP, or +55 XP with the Training Yard upgrade. Can level up." },
  { focusId: "stat_coaching", name: "Stat Coaching", category: "Stats", description: "Leave a creature for a longer, focused coaching plan aimed at its weakest stat grade.", costGold: 180, durationDays: 3, iconPath: "/images/ui/icons/icon_stat_growth.png", effectLabel: "Takes 3 days. On return: 18% chance, or 25% with the Coaching Bench, to improve the lowest stat grade by one rank and add +1 to that stat." },
...
export function getTrainingReturnSummaryItems(save: GameSave): string[] { const active = (save.creatures ?? []).flatMap((creature) => { const assignment = getTrainingAssignment(save, creature.creatureId); return assignment ? [{ creature, assignment }] : []; }); const ready = active.filter((entry) => entry.assignment.isReady).map((entry) => { const focus = getTrainingFocus(entry.assignment.focusId); const target = entry.assignment.targetStatKey ? ` (${entry.assignment.targetStatKey})` : ""; return `${entry.creature.nickname} returns from ${focus?.name ?? "training"}${target} today. Visit Rhea at the Training Grounds to collect the result.`; }); const awayCount = active.filter((entry) => !entry.assignment.isReady).length; return awayCount > 0 ? [...ready, `${awayCount} creature${awayCount === 1 ? " is" : "s are"} still away at the Training Grounds.`] : ready; }
...
export function getLatestTrainingResultSummary(save: GameSave, creatureId: CreatureId): string | null { const result = save.flags[`trainingGroundsLastResult_${creatureId}`]; return typeof result === "string" && result.trim() ? result.trim() : null; }
export function getRanchMaterialStock(save: GameSave): number { return getFlagNumber(save.flags.ranchMaterialsStock); }
...
export function purchaseTrainingUpgrade(save: GameSave, upgradeId: TrainingUpgradeId): TrainingResult {
  ...
  if (upgrade.requiredUpgradeId && !hasTrainingUpgrade(save, upgrade.requiredUpgradeId)) return { save, ok: false, message: `${upgrade.name} requires ${getTrainingUpgrade(upgrade.requiredUpgradeId)?.name ?? "another Training Grounds upgrade"} first.` };
  const materialStock = getRanchMaterialStock(save);
  if (save.currencies.gold < upgrade.costGold) return { save, ok: false, message: `Need ${upgrade.costGold} Gold for ${upgrade.name}.` };
  if (materialStock < upgrade.materialCost) return { save, ok: false, message: `Need ${upgrade.materialCost} Materials for ${upgrade.name}. Current stock: ${materialStock}.` };
  return { save: { ...save, updatedAt: new Date().toISOString(), currencies: { ...save.currencies, gold: save.currencies.gold - upgrade.costGold }, flags: { ...save.flags, m49TrainingUpgrades: true, m50TrainingPolishEconomy: true, ranchMaterialsStock: Math.max(0, materialStock - upgrade.materialCost), [getTrainingUpgradeFlag(upgradeId)]: true } }, ok: true, message: `${upgrade.name} installed. ${upgrade.effectLabel}` };
}
