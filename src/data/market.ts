...
import { getTamsinAdoptionFeeMultiplier, getTamsinArrivalRefreshMultiplier, getTamsinSpecialPlacementBonus, grantNpcTrust } from "@/data/townNpcs";
...
function chooseMarketVariant(save: GameSave, slotIndex: number, rerollCount: number): VariantId {
  ...
  const specialChance = effects.marketVariantChance + getTamsinSpecialPlacementBonus(save);
  if (specialChance > 0 && roll < specialChance) { const specialIndex = Math.floor(seededNumber(seed * 5 + 97) * MARKET_SPECIAL_VARIANTS.length); return MARKET_SPECIAL_VARIANTS[specialIndex] ?? "variant_unicorn" as VariantId; }
...
export function getMarketListingPrice(save: GameSave, listing: MarketListing): number {
  return Math.max(1, Math.round((listing.price * getTamsinAdoptionFeeMultiplier(save)) / 10) * 10);
}
...
export function ensureCurrentMarketState(save: GameSave): GameSave { const expectedCount = getTownUpgradeEffects(save).marketListingCount; if (save.market && save.market.weekNumber === save.dayState.weekNumber && save.market.listings.length === expectedCount) return save; return { ...save, market: createDefaultMarketState(save), flags: { ...save.flags, m35AdoptionHearthRenamed: true } }; }
export function getMarketRerollCost(save: GameSave): number { const market = ensureCurrentMarketState(save).market ?? createDefaultMarketState(save); const baseCost = BASE_REROLL_COST + market.rerollCount * 75; const discount = getTownUpgradeEffects(save).marketRerollDiscount; const tamsinDiscount = getTamsinArrivalRefreshMultiplier(save); return Math.max(25, Math.round((baseCost * (1 - discount) * tamsinDiscount) / 10) * 10); }
...
function createCreatureFromListing(save: GameSave, listing: MarketListing): CreatureRecord { const variant = getVariantDefinition(listing.variantId); const species = getSpeciesDefinition(variant.speciesId); const habitat = getHabitatForFamily(save, variant.family); const now = new Date().toISOString(); const creatureId = `creature_adoption_${Date.now()}_${listing.slotIndex}` as CreatureId; const level = 1; const preview = getMarketListingPreview(save, listing); const moveRarity = variant.rarity === "Rare" || variant.rarity === "Epic" ? "rare" : "common"; return { creatureId, ownerSaveId: save.saveId, speciesId: species.speciesId, variantId: variant.variantId, habitatId: (habitat?.habitatId ?? `habitat_${variant.family}`) as HabitatId, nickname: variant.name, level, xp: 0, xpToNext: getCreatureXpToNext(level), stats: preview.stats, statGrades: preview.statGrades, abilities: preview.abilities, battleMoveLoadout: getStoreBattleMoveLoadout(species.speciesId, moveRarity), energy: preview.maxEnergy, maxEnergy: preview.maxEnergy, hearts: preview.maxHearts, maxHearts: preview.maxHearts, affection: 35, generation: 1, shiny: false, cosmeticVariant: null, origin: "market", originLabel: `Adopted from Vale's Adoption Hearth · Week ${save.dayState.weekNumber}`, isLocked: false, createdAt: now, notes: `Adopted through Tamsin Vale at Vale's Adoption Hearth during week ${save.dayState.weekNumber}.` }; }

export function buyMarketListing(save: GameSave, listingId: string): MarketActionResult { const syncedSave = ensureCurrentMarketState(save); const market = syncedSave.market ?? createDefaultMarketState(syncedSave); const listing = market.listings.find((item) => item.listingId === listingId); if (!listing) return { save: syncedSave, ok: false, message: "That adoption listing no longer exists." }; if (listing.status === "sold") return { save: syncedSave, ok: false, message: "That creature has already been adopted." }; const price = getMarketListingPrice(syncedSave, listing); if (syncedSave.currencies.gold < price) return { save: syncedSave, ok: false, message: "Not enough Gold for this adoption fee." }; const habitat = getHabitatForFamily(syncedSave, listing.family); if (!habitat) return { save: syncedSave, ok: false, message: "No matching habitat is available for this creature." }; if (habitat.creatureIds.length >= habitat.capacity) return { save: syncedSave, ok: false, message: `${habitat.name} is full.` }; const creature = createCreatureFromListing(syncedSave, listing); const nextListings = market.listings.map((item) => item.listingId === listingId ? { ...item, status: "sold" as const } : item); const nextSave: GameSave = { ...syncedSave, updatedAt: new Date().toISOString(), currencies: { ...syncedSave.currencies, gold: syncedSave.currencies.gold - price }, creatureIds: [...syncedSave.creatureIds, creature.creatureId], creatures: [...(syncedSave.creatures ?? []), creature], habitats: (syncedSave.habitats ?? []).map((item) => item.habitatId === habitat.habitatId ? { ...item, creatureIds: [...item.creatureIds, creature.creatureId] } : item), market: { ...market, listings: nextListings }, flags: { ...syncedSave.flags, m6MarketPurchaseMade: true, m85MarketStatGrades: true, m9MarketOriginCreated: true, m13MarketContentPack: true, m18ScarceMarketAbilities: true, m35AdoptionHearthRenamed: true, m37TrustBonuses: true, m62MarketMoveKits: true, tamsinValeIntroduced: true } }; return { save: grantNpcTrust(nextSave, "tamsin_vale", 5), ok: true, message: `${creature.nickname} was adopted into ${habitat.name}. Tamsin Trust increased.` }; }

export function rerollMarketListings(save: GameSave): MarketActionResult { const syncedSave = ensureCurrentMarketState(save); const market = syncedSave.market ?? createDefaultMarketState(syncedSave); const cost = getMarketRerollCost(syncedSave); if (syncedSave.currencies.gold < cost) return { save: syncedSave, ok: false, message: "Not enough Gold to refresh arrivals." }; const nextRerollCount = market.rerollCount + 1; const rerolledSave: GameSave = { ...syncedSave, updatedAt: new Date().toISOString(), currencies: { ...syncedSave.currencies, gold: syncedSave.currencies.gold - cost }, market: { ...market, rerollCount: nextRerollCount, listings: createListings(syncedSave, nextRerollCount) }, flags: { ...syncedSave.flags, m6MarketRerolled: true, m35AdoptionHearthRenamed: true, m37TrustBonuses: true } }; return { save: grantNpcTrust(rerolledSave, "tamsin_vale", 1), ok: true, message: `New arrivals refreshed for ${cost} Gold. Tamsin Trust increased.` }; }
