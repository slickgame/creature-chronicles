...
function improveAbilityGrade(grade: CreatureAbility["grade"]): CreatureAbility["grade"] {
  const index = ABILITY_GRADE_ORDER.indexOf(grade);
  return ABILITY_GRADE_ORDER[Math.min(ABILITY_GRADE_ORDER.length - 1, Math.max(0, index) + 1)] ?? grade;
}

export function getNurserySupplyKitCount(save: GameSave): number { return getFlagNumber(save.flags.nurserySupplyKits); }
export function getEggAtelierAbilityPolishChance(save: GameSave): number { const trustLevel = getNpcTrustRecord(save, "selene_virell").level; return Math.min(42, 18 + trustLevel * 3); }
export function getEggAtelierServiceCost(serviceId: EggAtelierServiceId, save: GameSave): { gold: number; nurseryKits: number; label: string } { const trustLevel = getNpcTrustRecord(save, "selene_virell").level; if (serviceId === "accelerated_incubation") return { gold: trustLevel >= 3 ? 50 : 75, nurseryKits: 1, label: `${trustLevel >= 3 ? 50 : 75} Gold + 1 Nursery Supply Kit` }; return { gold: trustLevel >= 4 ? 125 : 150, nurseryKits: 1, label: `${trustLevel >= 4 ? 125 : 150} Gold + 1 Nursery Supply Kit` }; }
