...
function getParentCreature(save: GameSave, parent?: ParentSnapshot): CreatureRecord | undefined { return parent?.creatureId ? (save.creatures ?? []).find((creature) => creature.creatureId === parent.creatureId) : undefined; }
function getOffspringGeneration(save: GameSave, egg: EggRecord): number { const parentGenerations = [getParentCreature(save, egg.parents.giver)?.generation, getParentCreature(save, egg.parents.receiver)?.generation].filter((value): value is number => typeof value === "number"); return parentGenerations.length ? Math.max(...parentGenerations) + 1 : 2; }
function getParentCreatureIds(egg: EggRecord): CreatureId[] { return [egg.parents.giver.creatureId, egg.parents.receiver.creatureId].filter((value): value is CreatureId => Boolean(value)); }
function getParentDisplayNames(egg: EggRecord): string[] { return [egg.parents.giver.displayName, egg.parents.receiver.displayName].filter(Boolean); }
...
export function hatchEgg(save: GameSave, eggId: EggId, nickname?: string): { save: GameSave; creature: CreatureRecord } | null {
  ...
  const parentDisplayNames = getParentDisplayNames(egg);
  const parentCreatureIds = getParentCreatureIds(egg);
  const generation = getOffspringGeneration(save, egg);
  const lineageNote = `Generation ${generation} offspring of ${parentDisplayNames.join(" × ")}.`;
  const creature: CreatureRecord = { creatureId, ownerSaveId: save.saveId, speciesId: species.speciesId, variantId: variant.variantId, habitatId: egg.habitatId, nickname: nickname?.trim() || `${variant.name} Hatchling`, level, xp: 0, xpToNext: getCreatureXpToNext(level), stats: egg.projectedStats, statGrades: egg.projectedStatGrades ?? DEFAULT_STAT_GRADES, abilities: egg.projectedAbilities, energy: maxEnergy, maxEnergy, hearts: maxHearts, maxHearts, affection: 35, generation, parentCreatureIds, parentDisplayNames, lineageNote, shiny: false, cosmeticVariant: null, origin: "hatched", originLabel: `Hatched · ${parentDisplayNames.join(" × ")}`, isLocked: false, createdAt: new Date().toISOString(), notes: `Hatched from ${parentDisplayNames.join(" and ")}. ${lineageNote}` };
