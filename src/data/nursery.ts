...
function getHabitatIdForSpecies(speciesId: SpeciesId): HabitatId { return getSpeciesDefinition(speciesId).family === "feline" ? FELINE_HABITAT_ID : CANINE_HABITAT_ID; }

function parentSnapshot(participant: BreedingParticipant): ParentSnapshot {
  return { participantId: participant.participantId, creatureId: participant.creatureId, displayName: participant.displayName, familyLabel: participant.familyLabel, kind: participant.kind };
}

function averageStat(statKey: keyof CreatureStats, giver?: CreatureRecord, receiver?: CreatureRecord): number {
  const values = [giver?.stats[statKey], receiver?.stats[statKey]].filter((value): value is number => typeof value === "number");
  return values.length ? Math.round(values.reduce((sum, value) => sum + value, 0) / values.length) : 5;
}
...
function inheritGradeForStat(seed: string, statKey: keyof CreatureStats, giver?: CreatureRecord, receiver?: CreatureRecord): { grade: StatGrade; note: string | null } {
  ...
  if (roll >= 94) {
    const upgraded = shiftStatGrade(inheritedBase, 1);
    return { grade: upgraded, note: upgraded === inheritedBase ? `${statKey} inherited ${inheritedBase}.` : `${statKey} inherited ${inheritedBase} and upgraded to ${upgraded}.` };
  }

  if (roll <= 7) {
    const downgraded = shiftStatGrade(inheritedBase, -1);
    return { grade: downgraded, note: downgraded === inheritedBase ? `${statKey} inherited ${inheritedBase}.` : `${statKey} inherited ${inheritedBase} but downgraded to ${downgraded}.` };
  }

  return { grade: inheritedBase, note: `${statKey} inherited ${inheritedBase}.` };
}

function inheritStatGrades(seed: string, giver?: CreatureRecord, receiver?: CreatureRecord): { statGrades: StatGrades; notes: string[] } {
  const notes: string[] = [];
  const statGrades = STAT_KEYS.reduce((grades, key) => {
    const inherited = inheritGradeForStat(seed, key, giver, receiver);
    if (inherited.note) notes.push(inherited.note);
    return { ...grades, [key]: inherited.grade };
  }, {} as StatGrades);
  return { statGrades, notes };
}

function buildInheritedStats(seed: string, baseStats: CreatureStats, statGrades: StatGrades, giver?: CreatureRecord, receiver?: CreatureRecord): { stats: CreatureStats; notes: string[] } {
  const notes: string[] = [];
  const rawStats = STAT_KEYS.reduce((nextStats, statKey, index) => {
    const inheritedAverage = averageStat(statKey, giver, receiver);
    const gradeAdjustedBase = applyStatGrades(baseStats, statGrades)[statKey];
    const baseAnchor = Math.round((inheritedAverage + gradeAdjustedBase) / 2);
    const roll = deterministicRoll(`${seed}_${statKey}_${index}`, 100);
    let variance = 0;
    if (roll <= 7) { variance = -2; notes.push(`${statKey} rolled unusually low.`); }
    else if (roll <= 24) variance = -1;
    else if (roll >= 93) { variance = 2; notes.push(`${statKey} rolled unusually high.`); }
    else if (roll >= 76) variance = 1;
    return { ...nextStats, [statKey]: Math.max(1, baseAnchor + variance) };
  }, {} as CreatureStats);
  if (!notes.length) notes.push("Stats inherited normally from parent averages, stat grades, and minor RNG.");
  return { stats: rawStats, notes };
}
...
function buildInheritedAbilities(seed: string, speciesAbilities: CreatureAbility[], variantAbilities: CreatureAbility[], speciesId: SpeciesId, variantId: VariantId, giver?: CreatureRecord, receiver?: CreatureRecord): { abilities: CreatureAbility[]; notes: string[] } {
  ...
  if (inheritedPool.length && deterministicRoll(`${seed}_ability_inherit`, 100) >= 62) {
  ...
  if (deterministicRoll(`${seed}_ability_mutation`, 100) >= 94) {
    const mutationPool = [...speciesAbilities, ...variantAbilities].filter((ability) => !chosen.some((chosenAbility) => chosenAbility.id === ability.id));
...
export function createInheritancePreview(save: GameSave, giverParticipant: BreedingParticipant, receiverParticipant: BreedingParticipant, seed: string): InheritancePreview {
  ...
  return {
    ...
    projectedStatGrades: gradeResult.statGrades,
    projectedAbilities: abilityResult.abilities,
    statRollNotes: [...gradeResult.notes, ...statResult.notes],
...
export function createPregnancyRecord(save: GameSave, giverParticipant: BreedingParticipant, receiverParticipant: BreedingParticipant, seed: string): PregnancyRecord {
  ...
  return { pregnancyId, createdAtDayNumber: save.dayState.dayNumber, createdAt: new Date().toISOString(), daysRemaining: 1, totalDays: 1, status: "pregnant", giver: parentSnapshot(giverParticipant), receiver: parentSnapshot(receiverParticipant), inheritance };
}
...
function createEggFromPregnancy(save: GameSave, pregnancy: PregnancyRecord): EggRecord {
  ...
  return {
    eggId, ownerSaveId: save.saveId, createdAtDayNumber: save.dayState.dayNumber, createdAt: new Date().toISOString(), daysRemaining: 2, totalDays: 2, status: "incubating", rarity: variant.rarity,
    speciesId: pregnancy.inheritance.projectedSpeciesId, variantId: pregnancy.inheritance.projectedVariantId, habitatId: getHabitatIdForSpecies(pregnancy.inheritance.projectedSpeciesId), parents: { giver: pregnancy.giver, receiver: pregnancy.receiver },
    projectedStats: pregnancy.inheritance.projectedStats,
    projectedStatGrades: pregnancy.inheritance.projectedStatGrades,
...
export function advanceNurseryDay(save: GameSave): { save: GameSave; summaryItems: string[] } {
  ...
  const nextPregnancies = (save.pregnancies ?? []).map((pregnancy) => {
    ...
    if (nextDaysRemaining <= 0) {
      ...
      return { ...pregnancy, daysRemaining: 0, status: "delivered" as const };
    }
    summaryItems.push(`${pregnancy.receiver.displayName}'s pregnancy timer advanced.`);
    return { ...pregnancy, daysRemaining: nextDaysRemaining };
  });
  const nextEggs = [...deliveredEggs, ...(save.eggs ?? [])].map((egg) => {
    if (egg.status !== "incubating") return egg;
    const nextDaysRemaining = Math.max(0, egg.daysRemaining - 1);
    if (nextDaysRemaining <= 0) return { ...egg, daysRemaining: 0, status: "ready" as const };
    return { ...egg, daysRemaining: nextDaysRemaining };
  });
  if (nextEggs.some((egg) => egg.status === "ready")) summaryItems.push("An egg is ready to hatch in the nursery.");
  return { save: { ...save, pregnancies: nextPregnancies, eggs: nextEggs, eggIds: nextEggs.map((egg) => egg.eggId), flags: { ...save.flags, m5NurseryTimersAdvanced: true, m85EggGradeInheritance: true } }, summaryItems };
}
...
export function removeEgg(save: GameSave, eggId: EggId, mode: "release" | "donate"): GameSave {
  ...
  return { ...save, eggs: nextEggs, eggIds: nextEggs.map((egg) => egg.eggId), currencies: mode === "donate" ? { ...save.currencies, gold: save.currencies.gold + 75, guildPoints: save.currencies.guildPoints + 1 } : save.currencies, flags: { ...save.flags, m5EggRemoved: mode } };
}
