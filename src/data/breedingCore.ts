...
export function performBreedingAttempt(save: GameSave, giverId: string, receiverId: string): { save: GameSave; attempt: BreedingAttemptRecord } | null {
  ...
  return { attempt, save: { ...save, player: nextPlayer, currencies: { ...save.currencies, energy: nextEnergy }, creatures: nextCreatures, pregnancies: pregnancy ? [pregnancy, ...(save.pregnancies ?? [])] : save.pregnancies ?? [], eggs: preservedEggs, eggIds: preservedEggs.map((egg) => egg.eggId), breeding: { ...breeding, attempts: [attempt, ...breeding.attempts].slice(0, 50), streaks: nextStreaks }, flags: nextFlags } };
}
