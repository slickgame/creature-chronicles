...
export function getCreatureMaxEnergyFromStats(stats: CreatureStats, variantId?: string, level = 1, statGrades: StatGrades = DEFAULT_STAT_GRADES): number { return 70 + Math.floor(stats.STA * 3.5) + (variantId ? getVariantMaxEnergyBonus(variantId as never) : 0) + Math.floor(Math.max(0, level - 1) * ({ D: 0.75, C: 1.25, B: 1.75, A: 2.25, S: 3 }[statGrades.STA] ?? 0.75)); }
...
function getDailyBreedingComfortBonus(save: GameSave): number { const rawValue = save.flags.ranchBreedingComfortBonusToday; const parsed = typeof rawValue === "number" ? rawValue : Number(rawValue ?? 0); return Number.isFinite(parsed) ? Math.max(0, Math.min(25, Math.floor(parsed))) : 0; }
...
function buildUnavailableReason(energy: number, hearts: number, isInjured: boolean, injuryLabel?: string | null): string | null { if (isInjured) return injuryLabel ? `${injuryLabel}; recovering` : "Injured; recovering"; if (energy < 12) return "Energy too low"; if (hearts < 1) return "No Hearts remaining"; return null; }
...
export function getBreedingPreview(save: GameSave, giverId: string | null, receiverId: string | null): BreedingPreview | null {
  ...
  const abilityBonus = giverEffects.pregnancyChance + receiverEffects.pregnancyChance + ranchEffects.breedingPregnancyBonus + dailyComfortBonus;
...
function applyPlayerProgression(player: PlayerProfile, xpGain: number, heartCost: number): { player: PlayerProfile; event: BreedingProgressionEvent } {
  ...
  while (xp >= xpToNext && levelUps < 20) { xp -= xpToNext; rank += 1; levelUps += 1; xpToNext = getBreederXpToNext(rank); const key = STAT_KEYS[(rank + levelUps) % STAT_KEYS.length]; stats = { ...stats, [key]: stats[key] + Math.max(1, Math.round((PLAYER_GROWTH_PROFILE[key] ?? 10) / 20)) }; }
