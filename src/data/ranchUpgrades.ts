...
export function getRanchUpgradeEffects(save: GameSave): RanchUpgradeEffects {
  ...
  return {
    felineCapacity: BASE_FELINE_CAPACITY + (habitatCapacityBonusByTier[felineTier] ?? 0),
    canineCapacity: BASE_CANINE_CAPACITY + (habitatCapacityBonusByTier[canineTier] ?? 0),
