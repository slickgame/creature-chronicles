...
export function getRanchUpgradeDefinition(upgradeId: RanchUpgradeId): RanchUpgradeDefinition { const definition = RANCH_UPGRADE_DEFINITIONS.find((upgrade) => upgrade.upgradeId === upgradeId); if (!definition) throw new Error(`Unknown ranch upgrade: ${upgradeId}`); return definition; }
...
export function getTotalRanchUpgradeTiers(save: GameSave): number { const upgrades = getRanchUpgrades(save); return RANCH_UPGRADE_DEFINITIONS.reduce((total, definition) => total + (upgrades[definition.upgradeId] ?? 0), 0); }
...
export function repairRanchDamage(save: GameSave): RanchUpgradePurchaseResult {
  ...
  if (currentDamage <= 0) return { save, ok: false, message: "The ranch is already in good condition. No repairs needed." };
  if (materialsStock < RANCH_REPAIR_MATERIAL_COST) return { save, ok: false, message: `Not enough Materials for repairs. Need ${RANCH_REPAIR_MATERIAL_COST}; you have ${materialsStock}. Assign Field Hauling to gather more.` };
