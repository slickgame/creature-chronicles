...
import { normalizeTalentInstances } from "@/data/talents/talentDefinitions";
...
export function rollScarceStarterAbilities(seed: string, speciesId: SpeciesId, variantId: VariantId): CreatureAbility[] {
  if (deterministicRoll(`${seed}_starter_ability`, 100) >= STARTER_ABILITY_CHANCE) return [];
  const ability = pickAbility(`${seed}_starter`, getAbilityPool(speciesId, variantId, false), 0);
  return normalizeTalentInstances(ability ? [ability] : []);
}

export function rollMarketAbilitiesForTier(seed: string, speciesId: SpeciesId, variantId: VariantId, marketQualityTier: number): CreatureAbility[] {
  const variant = getVariantDefinition(variantId);
  const tier = Math.max(0, Math.min(4, marketQualityTier));
  const rarityBonus = variant.rarity === "Rare" ? 3 : variant.rarity === "Epic" ? 6 : variant.rarity === "Uncommon" ? 1 : 0;
  const chance = (MARKET_ABILITY_CHANCE_BY_TIER[tier] ?? MARKET_ABILITY_CHANCE_BY_TIER[0]) + rarityBonus;
  if (deterministicRoll(`${seed}_market_ability`, 100) >= chance) return [];
  const pool = getAbilityPool(speciesId, variantId, tier >= 2);
  const first = pickAbility(`${seed}_market_first`, pool, tier);
  const abilities = first ? [first] : [];
  const secondChance = MARKET_SECOND_ABILITY_CHANCE_BY_TIER[tier] ?? 0;
  if (abilities.length && deterministicRoll(`${seed}_market_second`, 100) < secondChance) {
    const second = pickAbility(`${seed}_market_second_pick`, pool, tier, abilities);
    if (second) abilities.push(second);
  }
  return normalizeTalentInstances(abilities);
}

export function rollMarketListingAbilities(save: GameSave, listing: MarketListing, seed: string, speciesId: SpeciesId, variantId: VariantId): CreatureAbility[] {
  return rollMarketAbilitiesForTier(seed, speciesId, variantId, getTownUpgradeEffects(save).marketQualityTier);
}

export function rebalanceExistingCreatureAbilities(creature: CreatureRecord, seed: string, speciesId: SpeciesId, variantId: VariantId): CreatureAbility[] {
  const abilities = creature.origin === "starter" || creature.creatureId.includes("starter")
    ? rollScarceStarterAbilities(seed, speciesId, variantId)
    : creature.abilities ?? [];
  return normalizeTalentInstances(abilities);
}
