...
export function getNpcTrustRecord(save: GameSave, npcId: TownNpcId): TownNpcTrustRecord {
  ...
  return { npcId, points, level: getTrustLevel(points), introduced: Boolean(current?.introduced), lastChangedDayNumber: current?.lastChangedDayNumber };
}

export function getTownNpcTrustState(save: GameSave): TownNpcTrustState {
  return (Object.keys(TOWN_NPCS) as TownNpcId[]).reduce<TownNpcTrustState>((state, npcId) => ({ ...state, [npcId]: getNpcTrustRecord(save, npcId) }), {});
}
...
export function grantNpcTrust(save: GameSave, npcId: TownNpcId, amount: number, introduced = true): GameSave {
  ...
  const nextRecord: TownNpcTrustRecord = { npcId, points, level: getTrustLevel(points), introduced: current.introduced || introduced, lastChangedDayNumber: save.dayState.dayNumber };
  return { ...save, townNpcTrust: { ...(save.townNpcTrust ?? {}), [npcId]: nextRecord }, flags: { ...save.flags, [`trust_${npcId}`]: points, [`trustLevel_${npcId}`]: nextRecord.level, m36TownNpcTrust: true } };
}
