...
export function GameProvider({ children }: { children: React.ReactNode }) {
  ...
  const renameCreature = useCallback((creatureId: CreatureId, nickname: string) => {
    ...
    saveCurrentGame({ ...currentSave, creatures: (currentSave.creatures ?? []).map((creature) => creature.creatureId === creatureId ? { ...creature, nickname: nickname.trim() } : creature), flags: { ...currentSave.flags, m3CreatureRenamed: true, m9RenamePolishUsed: true } });
  }, [currentSave, saveCurrentGame]);
  ...
  const advanceDay = useCallback((): DayAdvanceResult | null => {
    ...
    const restoredSave: GameSave = {
      ...
      creatures: (currentSave.creatures ?? []).map((creature) => ({ ...creature, energy: Math.min(creature.maxEnergy + recovery.sleepCreatureEnergyBonus, creature.maxEnergy + recovery.sleepCreatureEnergyBonus), hearts: creature.maxHearts ?? 4, affection: Math.min(100, creature.affection + recovery.sleepAffectionBonus) })),
