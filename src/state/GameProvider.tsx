...
import { collectTrainingGroundsAssignment, getTrainingReturnSummaryItems, getTrainingUnavailableReason, startTrainingGroundsAssignment } from "@/data/trainingGrounds";
...
import type { TrainingFocusId, TrainingResult } from "@/data/trainingGrounds";
...
export function GameProvider({ children }: { children: React.ReactNode }) {
  ...
  const trainCreature = useCallback((creatureId: CreatureId, focusId: TrainingFocusId) => { if (!currentSave) return { save: currentSave as unknown as GameSave, ok: false, message: "No active save." }; return startTrainingGroundsAssignment(currentSave, creatureId, focusId); }, [currentSave]);
  ...
  const value = useMemo<GameContextValue>(() => ({ version: MVP_VERSION, buildPhase: "M47 — Training Availability", appScreen, activeHabitatFamily, currentSave, saveSlots, isHydrated, createNewGame, loadGame, deleteGame, refreshSaveSlots, goToMainMenu, exitRunToMainMenu, goToRanch, goToHabitat, goToBreeding, goToNursery, goToTown, goToMarket, goToSupplyDepot, goToEggAtelier, goToTrainingGrounds, goToGuildHall, goToCollection, goToRanchOffice, goToRanchJobs, goToDevTools, saveCurrentGame, advanceDay, renameCreature, feedCreature, toggleCreatureLock, releaseCreature, donateCreature, attemptBreeding, hatchReadyEgg, removeNurseryEgg, buyMarketCreature, rerollMarket, buySupplyDepotItem, trainCreature, collectTrainingCreature, acceptGuildRequest, donateCreatureToGuild, buyTownUpgrade, buyRanchUpgrade, repairRanch, assignRanchJob, claimGuildIntroBonus, addDevGuildPoints }), [appScreen, activeHabitatFamily, currentSave, saveSlots, isHydrated, createNewGame, loadGame, deleteGame, refreshSaveSlots, goToMainMenu, exitRunToMainMenu, goToRanch, goToHabitat, goToBreeding, goToNursery, goToTown, goToMarket, goToSupplyDepot, goToEggAtelier, goToTrainingGrounds, goToGuildHall, goToCollection, goToRanchOffice, goToRanchJobs, goToDevTools, saveCurrentGame, advanceDay, renameCreature, feedCreature, toggleCreatureLock, releaseCreature, donateCreature, attemptBreeding, hatchReadyEgg, removeNurseryEgg, buyMarketCreature, rerollMarket, buySupplyDepotItem, trainCreature, collectTrainingCreature, acceptGuildRequest, donateCreatureToGuild, buyTownUpgrade, buyRanchUpgrade, repairRanch, assignRanchJob, claimGuildIntroBonus, addDevGuildPoints]);
