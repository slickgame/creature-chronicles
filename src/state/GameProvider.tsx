...
function getNextDayState(dayState: DayState): DayState {
  ...
  return { dayNumber: nextDayNumber, weekday: WEEKDAYS[nextWeekdayIndex], month: nextMonth, dayOfMonth: nextDayOfMonth, weekNumber: nextWeekNumber };
}
...
export function GameProvider({ children }: { children: React.ReactNode }) {
  ...
  const renameCreature = useCallback((creatureId: CreatureId, nickname: string) => {
    ...
    saveCurrentGame({ ...currentSave, creatures: (currentSave.creatures ?? []).map((creature) => creature.creatureId === creatureId ? { ...creature, nickname: cleanNickname } : creature), flags: { ...currentSave.flags, m3CreatureRenamed: true, m9RenamePolishUsed: true } });
  }, [currentSave, saveCurrentGame]);

  const feedCreature = useCallback((creatureId: CreatureId) => {
    if (!currentSave) return;
    saveCurrentGame({ ...currentSave, creatures: (currentSave.creatures ?? []).map((creature) => creature.creatureId === creatureId ? { ...creature, affection: Math.min(100, creature.affection + 5), energy: Math.min(creature.maxEnergy, creature.energy + 10) } : creature), flags: { ...currentSave.flags, m3CreatureFed: true } });
  }, [currentSave, saveCurrentGame]);

  const toggleCreatureLock = useCallback((creatureId: CreatureId) => {
    if (!currentSave) return;
    saveCurrentGame({ ...currentSave, creatures: (currentSave.creatures ?? []).map((creature) => creature.creatureId === creatureId ? { ...creature, isLocked: !creature.isLocked } : creature), flags: { ...currentSave.flags, m9CreatureLockUsed: true } });
  }, [currentSave, saveCurrentGame]);
  ...
  const advanceDay = useCallback((): DayAdvanceResult | null => {
    ...
    const restoredSave: GameSave = { ...currentSave, updatedAt: new Date().toISOString(), dayState: nextDayState, player: { ...currentSave.player, hearts: currentSave.player.maxHearts ?? 4 }, currencies: { ...currentSave.currencies, energy: currentSave.currencies.maxEnergy }, creatures: (currentSave.creatures ?? []).map((creature) => ({ ...creature, energy: creature.maxEnergy, hearts: creature.maxHearts ?? 4 })), breeding: currentSave.breeding, pregnancies: currentSave.pregnancies ?? [], eggs: currentSave.eggs ?? [], market: currentSave.market, guild: currentSave.guild, townUpgrades: currentSave.townUpgrades, flags: { ...currentSave.flags, lastSleptDayNumber: nextDayState.dayNumber, m2SleepUsed: true } };
  ...
  const value = useMemo<GameContextValue>(() => ({
    version: MVP_VERSION,
    buildPhase: "M10.5 — Guild Economy Polish",
    appScreen,
    activeHabitatFamily,
    currentSave,
    saveSlots,
    ...
    addDevGuildPoints,
  }), [appScreen, activeHabitatFamily, currentSave, saveSlots, isHydrated, createNewGame, loadGame, deleteGame, refreshSaveSlots, goToMainMenu, goToRanch, goToHabitat, goToBreeding, goToNursery, goToTown, goToMarket, goToGuildHall, goToCollection, saveCurrentGame, advanceDay, renameCreature, feedCreature, toggleCreatureLock, releaseCreature, donateCreature, attemptBreeding, hatchReadyEgg, removeNurseryEgg, buyMarketCreature, rerollMarket, acceptGuildRequest, donateCreatureToGuild, buyTownUpgrade, claimGuildIntroBonus, addDevGuildPoints]);
