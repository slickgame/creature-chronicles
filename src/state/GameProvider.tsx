...
import { collectTrainingGroundsAssignment, startTrainingGroundsAssignment } from "@/data/trainingGrounds";
...
export function GameProvider({ children }: { children: React.ReactNode }) {
  ...
  const advanceDay = useCallback((): DayAdvanceResult | null => {
    ...
    const restoredSave: GameSave = { ...currentSave, updatedAt: new Date().toISOString(), dayState: nextDayState, player: { ...currentSave.player, hearts: currentSave.player.maxHearts ?? 4 }, currencies: { ...currentSave.currencies, energy: currentSave.currencies.maxEnergy }, creatures: (currentSave.creatures ?? []).map((creature) => ({ ...creature, energy: Math.min(creature.maxEnergy + recovery.sleepCreatureEnergyBonus, creature.maxEnergy + recovery.sleepCreatureEnergyBonus), hearts: creature.maxHearts ?? 4, affection: Math.min(100, creature.affection + recovery.sleepAffectionBonus) })), breeding: currentSave.breeding, pregnancies: currentSave.pregnancies ?? [], eggs: currentSave.eggs ?? [], market: currentSave.market, guild: currentSave.guild, townUpgrades: currentSave.townUpgrades, townNpcTrust: currentSave.townNpcTrust, ranchUpgrades: currentSave.ranchUpgrades, ranchJobs: currentSave.ranchJobs, flags: { ...currentSave.flags, lastSleptDayNumber: nextDayState.dayNumber, m2SleepUsed: true, m11SleepRecoveryApplied: recovery.sleepCreatureEnergyBonus > 0 || recovery.sleepAffectionBonus > 0 } };
