...
import { purchaseBattleOutfitterItem } from "@/data/battleOutfitter";
...
export type AppScreen = "main-menu" | "ranch-hub" | "habitat" | "breeding" | "nursery" | "town" | "market" | "supply-depot" | "egg-atelier" | "training-grounds" | "battle-outfitter" | "guild-hall" | "collection" | "ranch-office" | "ranch-jobs" | "dev-tools";
...
type GameContextValue = {
  ...
  goToMainMenu: () => void; exitRunToMainMenu: () => void; goToRanch: () => void; goToHabitat: (family: CreatureFamily) => void; goToBreeding: () => void; goToNursery: () => void; goToTown: () => void; goToMarket: () => void; goToSupplyDepot: () => void; goToEggAtelier: () => void; goToTrainingGrounds: () => void; goToBattleOutfitter: () => void; goToGuildHall: () => void; goToCollection: () => void; goToRanchOffice: () => void; goToRanchJobs: () => void; goToDevTools: () => void;
  saveCurrentGame: (nextSave: GameSave) => GameSave; advanceDay: () => DayAdvanceResult | null; renameCreature: (creatureId: CreatureId, nickname: string) => void; feedCreature: (creatureId: CreatureId) => void; toggleCreatureLock: (creatureId: CreatureId) => void; releaseCreature: (creatureId: CreatureId) => string; donateCreature: (creatureId: CreatureId) => string; attemptBreeding: (giverId: string, receiverId: string) => BreedingAttemptRecord | null; hatchReadyEgg: (eggId: EggId, nickname?: string) => CreatureRecord | null; removeNurseryEgg: (eggId: EggId, mode: "release" | "donate") => void; buyMarketCreature: (listingId: string) => string; rerollMarket: () => string; buySupplyDepotItem: (itemId: string) => string; buyBattleOutfitterItem: (itemId: string) => BattleOutfitterResult; trainCreature: (creatureId: CreatureId, focusId: TrainingFocusId, targetStatKey?: CreatureStatKey) => TrainingResult; collectTrainingCreature: (creatureId: CreatureId) => TrainingResult; purchaseTrainingGroundsUpgrade: (upgradeId: TrainingUpgradeId) => TrainingResult; acceptGuildRequest: (contractId: string) => string; donateCreatureToGuild: (contractId: string, creatureId: CreatureId) => string; buyTownUpgrade: (upgradeId: TownUpgradeId) => TownUpgradePurchaseResult; buyRanchUpgrade: (upgradeId: RanchUpgradeId) => RanchUpgradePurchaseResult; repairRanch: () => RanchUpgradePurchaseResult; assignRanchJob: (jobId: RanchJobId, creatureId: CreatureId | null) => RanchJobAssignmentResult; claimGuildIntroBonus: () => TownUpgradePurchaseResult; addDevGuildPoints: () => TownUpgradePurchaseResult;
};
...
export function GameProvider({ children }: { children: React.ReactNode }) {
  ...
  const goToEggAtelier = useCallback(() => { setActiveHabitatFamily(null); setAppScreen("egg-atelier"); }, []);
  const goToTrainingGrounds = useCallback(() => { setActiveHabitatFamily(null); setAppScreen("training-grounds"); }, []);
  const goToBattleOutfitter = useCallback(() => { setActiveHabitatFamily(null); setAppScreen("battle-outfitter"); }, []);
  ...
  const feedCreature = useCallback((creatureId: CreatureId) => { if (!currentSave) return; if (getTrainingUnavailableReason(currentSave, creatureId)) return; saveCurrentGame({ ...currentSave, creatures: (currentSave.creatures ?? []).map((creature) => creature.creatureId === creatureId ? { ...creature, affection: Math.min(100, creature.affection + 5), energy: Math.min(creature.maxEnergy, creature.energy + 10) } : creature), flags: { ...currentSave.flags, m3CreatureFed: true } }); }, [currentSave, saveCurrentGame]);
  ...
  const hatchReadyEgg = useCallback((eggId: EggId, nickname?: string) => { if (!currentSave) return null; const result = hatchEgg(currentSave, eggId, nickname); if (!result) return null; const atelierResult = applyEggAtelierHatchEffects(currentSave, result, eggId); saveCurrentGame(atelierResult.save); return atelierResult.creature; }, [currentSave, saveCurrentGame]);
  ...
  const buyBattleOutfitterItem = useCallback((itemId: string) => { if (!currentSave) return { save: currentSave as unknown as GameSave, ok: false, message: "No active save." }; const result = purchaseBattleOutfitterItem(currentSave, itemId); if (result.ok) saveCurrentGame(result.save); return result; }, [currentSave, saveCurrentGame]);
  const trainCreature = useCallback((creatureId: CreatureId, focusId: TrainingFocusId, targetStatKey?: CreatureStatKey) => { if (!currentSave) return { save: currentSave as unknown as GameSave, ok: false, message: "No active save." }; return startTrainingGroundsAssignment(currentSave, creatureId, focusId, targetStatKey); }, [currentSave]);
  const collectTrainingCreature = useCallback((creatureId: CreatureId) => { if (!currentSave) return { save: currentSave as unknown as GameSave, ok: false, message: "No active save." }; return collectTrainingGroundsAssignment(currentSave, creatureId); }, [currentSave]);
  const purchaseTrainingGroundsUpgrade = useCallback((upgradeId: TrainingUpgradeId) => { if (!currentSave) return { save: currentSave as unknown as GameSave, ok: false, message: "No active save." }; return purchaseTrainingUpgrade(currentSave, upgradeId); }, [currentSave]);
  ...
  const advanceDay = useCallback((): DayAdvanceResult | null => {
    ...
    const restoredSave: GameSave = { ...currentSave, updatedAt: new Date().toISOString(), dayState: nextDayState, player: { ...currentSave.player, hearts: currentSave.player.maxHearts ?? 4 }, currencies: { ...currentSave.currencies, energy: currentSave.currencies.maxEnergy }, creatures: (currentSave.creatures ?? []).map((creature) => getTrainingUnavailableReason(currentSave, creature.creatureId) ? creature : ({ ...creature, energy: Math.min(creature.maxEnergy + recovery.sleepCreatureEnergyBonus, creature.maxEnergy + recovery.sleepCreatureEnergyBonus), hearts: creature.maxHearts ?? 4, affection: Math.min(100, creature.affection + recovery.sleepAffectionBonus) })), breeding: currentSave.breeding, pregnancies: currentSave.pregnancies ?? [], eggs: currentSave.eggs ?? [], market: currentSave.market, guild: currentSave.guild, townUpgrades: currentSave.townUpgrades, townNpcTrust: currentSave.townNpcTrust, ranchUpgrades: currentSave.ranchUpgrades, ranchJobs: currentSave.ranchJobs, flags: { ...currentSave.flags, lastSleptDayNumber: nextDayState.dayNumber, m2SleepUsed: true, m11SleepRecoveryApplied: recovery.sleepCreatureEnergyBonus > 0 || recovery.sleepAffectionBonus > 0, m47TrainingAvailability: true } };
  ...
  const value = useMemo<GameContextValue>(() => ({ version: MVP_VERSION, buildPhase: "M51 — Battle Outfitter", appScreen, activeHabitatFamily, currentSave, saveSlots, isHydrated, createNewGame, loadGame, deleteGame, refreshSaveSlots, goToMainMenu, exitRunToMainMenu, goToRanch, goToHabitat, goToBreeding, goToNursery, goToTown, goToMarket, goToSupplyDepot, goToEggAtelier, goToTrainingGrounds, goToBattleOutfitter, goToGuildHall, goToCollection, goToRanchOffice, goToRanchJobs, goToDevTools, saveCurrentGame, advanceDay, renameCreature, feedCreature, toggleCreatureLock, releaseCreature, donateCreature, attemptBreeding, hatchReadyEgg, removeNurseryEgg, buyMarketCreature, rerollMarket, buySupplyDepotItem, buyBattleOutfitterItem, trainCreature, collectTrainingCreature, purchaseTrainingGroundsUpgrade, acceptGuildRequest, donateCreatureToGuild, buyTownUpgrade, buyRanchUpgrade, repairRanch, assignRanchJob, claimGuildIntroBonus, addDevGuildPoints }), [appScreen, activeHabitatFamily, currentSave, saveSlots, isHydrated, createNewGame, loadGame, deleteGame, refreshSaveSlots, goToMainMenu, exitRunToMainMenu, goToRanch, goToHabitat, goToBreeding, goToNursery, goToTown, goToMarket, goToSupplyDepot, goToEggAtelier, goToTrainingGrounds, goToBattleOutfitter, goToGuildHall, goToCollection, goToRanchOffice, goToRanchJobs, goToDevTools, saveCurrentGame, advanceDay, renameCreature, feedCreature, toggleCreatureLock, releaseCreature, donateCreature, attemptBreeding, hatchReadyEgg, removeNurseryEgg, buyMarketCreature, rerollMarket, buySupplyDepotItem, buyBattleOutfitterItem, trainCreature, collectTrainingCreature, purchaseTrainingGroundsUpgrade, acceptGuildRequest, donateCreatureToGuild, buyTownUpgrade, buyRanchUpgrade, repairRanch, assignRanchJob, claimGuildIntroBonus, addDevGuildPoints]);
