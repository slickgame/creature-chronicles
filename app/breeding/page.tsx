...
type PresetSortMode = "custom" | "best_match";
...
export default function BreedingPage() {
  ...
  function getPresetScore(preset: BreedingPreset) {
    ...
    const giverReady = presetGiver ? isCreatureReady(presetGiver) : true;
    const receiverReady = presetReceiver ? isCreatureReady(presetReceiver) : true;
    ...
    const giverSnapshot =
      preset.giverType === "player"
        ...
        : presetGiver
        ? {
            happiness: presetGiver.happiness,
            fertility: presetGiver.stats.fertility,
            vitality: presetGiver.stats.vitality,
            intelligence: presetGiver.stats.intelligence,
            speed: presetGiver.stats.speed,
            breedingCareLevel: presetGiver.skills?.breedingCare?.level ?? 1,
            traits: Array.isArray(presetGiver.traits) ? presetGiver.traits : [],
          }
        : null;
    ...
    const receiverSnapshot =
      preset.receiverType === "player"
        ...
        : presetReceiver
        ? {
            happiness: presetReceiver.happiness,
            fertility: presetReceiver.stats.fertility,
            vitality: presetReceiver.stats.vitality,
            intelligence: presetReceiver.stats.intelligence,
            speed: presetReceiver.stats.speed,
            breedingCareLevel: presetReceiver.skills?.breedingCare?.level ?? 1,
            traits: Array.isArray(presetReceiver.traits) ? presetReceiver.traits : [],
          }
        : null;

    let avgHappiness = 60;
    let avgBreedingCare = 1;
    let eggChance = 0;
    let refusalChance = 0.1;

    const happinessValues = [giverSnapshot?.happiness, receiverSnapshot?.happiness].filter(
      (value): value is number => typeof value === "number"
    );
    ...
    const breedingCareValues = [
      giverSnapshot?.breedingCareLevel,
      receiverSnapshot?.breedingCareLevel,
    ].filter((value): value is number => typeof value === "number");
    if (breedingCareValues.length > 0) {
      avgBreedingCare =
        breedingCareValues.reduce((sum, value) => sum + value, 0) /
        breedingCareValues.length;
    }

    if (preset.receiverType !== "player") {
      const fertilities = [giverSnapshot?.fertility, receiverSnapshot?.fertility].filter(
        (value): value is number => typeof value === "number"
      );
      const vitalities = [giverSnapshot?.vitality, receiverSnapshot?.vitality].filter(
        (value): value is number => typeof value === "number"
      );
      ...
      eggChance =
        ...
        (avgFertility - 5) * 0.05 +
        (avgVitality - 5) * 0.02 +
        (avgHappiness - 50) * 0.003 +
        avgBreedingCare * 0.015;

      eggChance = Math.max(0.1, Math.min(0.95, eggChance));
    }
    ...
    let score = 0;
    if (validation.canLoad) score += 100;
    ...
    if (giverReady) score += 30;
    if (receiverReady) score += 30;
    score += Math.round(eggChance * 100);
    score -= Math.round(refusalChance * 100);

    let label = "Usable";
    if (!giverReady || !receiverReady) label = "Not Ready";
    if (validation.familyRisk === "half_sibling") label = "Risk";
    if (validation.familyRisk === "full_sibling" || validation.familyRisk === "parent_child")
      label = "High Risk";
  ...
  const sortedPresetSlots = useMemo(() => {
    ...
    return [...slots].sort((a, b) => {
      ...
      const scoreA = getPresetScore(presetA).score;
      const scoreB = getPresetScore(presetB).score;

      if (scoreB !== scoreA) return scoreB - scoreA;

      return a - b;
    });
  }, [presetSortMode, presets, creatures, playerData, homeState]);
              ...
              <div className="space-y-3 overflow-y-auto pr-1">
                ...
                <div className="rounded-2xl bg-rose-50 p-3">
                  ...
                  <div className="mb-3 rounded-2xl border border-rose-200 bg-white p-3">
                    ...
                    <p className="mt-2 text-xs text-stone-600">
                      {presetSortMode === "custom"
                        ? "Manual move controls are active."
                        : "Presets are temporarily ranked by usefulness for the current ranch state."}
                    </p>
                  </div>
                  ...
                  <div className="mb-3 rounded-2xl border border-emerald-300 bg-emerald-50 p-3 text-xs text-emerald-900">
                    Favorites and presets persist in your browser with localStorage.
                  </div>
                          ...
                          {preset ? (
                            <>
                                  ...
                                  {presetScore && (
                                    <div
                                      className={`rounded-full border px-2 py-1 text-[11px] font-semibold ${
                                        presetScore.label === "Strong Match"
                                          ? "border-emerald-300 bg-emerald-100 text-emerald-900"
                                          : presetScore.label === "Missing" ||
                                            presetScore.label === "Invalid"
                                          ? "border-red-300 bg-red-100 text-red-900"
                                          : presetScore.label === "Risk" ||
                                            presetScore.label === "High Risk"
                                          ? "border-amber-300 bg-amber-100 text-amber-900"
                                          : presetScore.label === "Not Ready"
                              ...
                              <div className="mt-2 flex flex-wrap gap-2">
                                ...
                                {presetScore && presetSortMode === "best_match" && (
                                  <div className="rounded-full border border-violet-300 bg-violet-100 px-2 py-1 text-[11px] font-semibold text-violet-900">
                              ...
                              {presetSortMode === "best_match" && (
                                <p className="mt-2 text-xs text-stone-500">
                                  Move buttons are disabled while Best Match sorting is active.
                                </p>
                              )}
                              ...
                              <div className="mt-2 flex flex-wrap gap-2">
                                ...
                                <button
                                  ...
                                  disabled={slot === PRESET_SLOT_COUNT || presetSortMode !== "custom"}
                                  className={`rounded-xl px-3 py-2 text-xs font-semibold ${
                                    slot < PRESET_SLOT_COUNT && presetSortMode === "custom"
                                      ? "border border-rose-300 bg-white text-stone-800"
                                      : "bg-stone-200 text-stone-500"
                                  }`}
                                >
