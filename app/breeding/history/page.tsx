...
function getQualityRank(quality: HistoryQuality) {
...
export default function BreedingHistoryPage() {
  ...
  const historyEntries = useMemo(() => {
    const eggEntries: HistoryEntry[] = eggs.map((egg) => ({
      ...
      notes: [
        egg.hatchDaysRemaining <= 0
          ? "This egg can be hatched now."
          : "This egg is still incubating.",
        egg.inbreedingRisk === "none"
          ? "No inherited family-risk penalty is expected."
          : "This egg carries family-risk penalty potential.",
      ],
    }));
  ...
  const filteredEntries = useMemo(() => {
    ...
    const filtered = historyEntries.filter((entry) => {
      ...
      const matchesFilter =
        ...
        (filter === "ready" &&
          entry.status === "egg" &&
          entry.hatchLabel === "Ready to hatch") ||
    ...
    filtered.sort((a, b) => {
      if (a.status !== b.status) {
        if (a.status === "egg") return -1;
        return 1;
      }

      if (filter === "risk") {
        return getRiskRank(b.risk) - getRiskRank(a.risk);
      }
  ...
  return (
    <>
          ...
          <div className="mb-6 flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
            <div>
              ...
              <p className="mt-1 text-stone-700">
                Compact history log for active eggs and hatched offspring.
              </p>
            </div>
          ...
          <div className="mb-6 rounded-3xl border-4 border-rose-900 bg-white/85 p-4 shadow-xl">
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
              <input
                ...
                className="rounded-2xl border border-rose-300 bg-white px-4 py-3 text-sm text-stone-800 lg:col-span-2"
              />
              <div className="rounded-2xl bg-rose-50 p-3 text-sm text-stone-800">
                <p><strong>Total Records:</strong> {historyEntries.length}</p>
              </div>
              <div className="rounded-2xl bg-rose-50 p-3 text-sm text-stone-800">
                <p><strong>Visible:</strong> {filteredEntries.length}</p>
              </div>
            </div>
          </div>
              ...
              filteredEntries.map((entry) => (
                ...
                >
                  ...
                  <div className="mt-2 flex flex-wrap gap-1.5">
                    ...
                    {entry.quality && (
                      <div
                        className={`inline-block rounded-full border px-2 py-1 text-[11px] font-semibold capitalize ${getQualityClasses(
                          entry.quality
                        )}`}
                      >
                        {entry.quality}
      ...
      <DetailModal
        ...
        title={selectedEntry ? selectedEntry.title : "History Details"}
        ...
        {selectedEntry && (
          <div className="space-y-4">
            <div className="rounded-2xl bg-rose-50 p-4">
              <div className="flex flex-wrap items-center gap-2">
                ...
                {selectedEntry.quality && (
                  <div
                    className={`rounded-full border px-3 py-1 text-sm font-semibold capitalize ${getQualityClasses(
                      selectedEntry.quality
                    )}`}
                  >
                    {selectedEntry.quality}
            ...
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <div className="rounded-2xl bg-stone-100 p-4">
                ...
                <div className="space-y-1 text-sm text-stone-800">
                  ...
                  {selectedEntry.hatchLabel && (
                    <p><strong>Hatch Status:</strong> {selectedEntry.hatchLabel}</p>
                  )}
                  {selectedEntry.quality && (
                    <p><strong>Quality Effect:</strong> {getEggQualityDescription(selectedEntry.quality)}</p>
                  )}
                </div>
              </div>
            </div>
            ...
            <div className="rounded-2xl bg-rose-50 p-4">
              ...
              <div className="space-y-2">
                {selectedEntry.notes.map((note, index) => (
                  <p key={`${selectedEntry.id}-note-${index}`} className="text-sm text-stone-800">
                    • {note}
                  </p>
                ))}
              </div>
            ...
            <div className="flex flex-wrap gap-3">
              <button
                type="button"
                onClick={() => loadPair(selectedEntry)}
                disabled={!selectedEntry.canReload}
                className={`rounded-2xl px-4 py-3 text-sm font-semibold shadow ${
                  selectedEntry.canReload
